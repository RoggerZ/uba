...
- `analytics-core` 的 P1-002 已启动：collect 请求标准化、`collect.Handler`、fasthttp `POST /collect`、storage `EventWriter` 接口、ClickHouse `TableRouter`、native batch `BatchWriter`、GORM/MySQL `IngestionStatusGuard`、`EventQueryBuilder` query plan、`storage.EventReader` 查询执行器和 `ingestion.Processor` worker 边界已落地；下一步不要绕开这些契约，端到端运行入口必须复用它们。
