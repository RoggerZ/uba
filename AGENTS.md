...
- 子代理代码审查必须同时执行 `$ai-slop-cleaner` 视角：检查重复逻辑、死代码、无必要抽象、弱边界、样板化实现、低价值注释、缺失回归测试和无关膨胀；该检查只针对本次变更文件，不能借机做大范围重写。
- 子代理代码审查应在提交前完成；发现必须修复的问题时，主代理先修复并重新验证，再按需追加复审。最终交付说明需要简要写明审查结果、`$ai-slop-cleaner` 结论或剩余风险。
