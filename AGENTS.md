...
- 文档同步以“提交点 / 阶段里程碑 / 明显决策变化”为主；不要在主线代码只前进一点点时就频繁打断去写大段文档。先把一个真实小闭环做完，再做最小必要的文档回填。
- 主线实施时优先把代码和可运行验证推进到一个完整小闭环，再集中同步文档；不要因为每个小代码改动频繁切换到 README、Q&A、源码解读或进度文档，导致实际代码推进变慢。
- 文档同步默认放在阶段检查点、准备提交前、决策状态变化、验收标准变化、出现 blocker 或用户明确要求时进行；普通小步迭代、局部修正、命名调整、测试断言调整和格式化不需要立刻刷新所有文档。
...
- 每次在实施决策、进度表、源码解读或卡壳记录中标注任务进度时，必须写明对应仓库的 git commit id；涉及子仓库时使用子仓库自己的 commit id，父仓进度使用父仓 commit id。如果相关代码或文档尚未提交，必须明确标注 `未提交` 或 `pending commit`，提交后立即回填真实 commit id。
...
- `analytics-core` 读侧长期规范已经明确：`EventQueryBuilder` 和 `EventReader` 是唯一读侧入口；先做属性治理和 query plan 约束，再按稳定查询引入 projection、materialized view 或小时聚合表；禁止在 handler、service 或产品页面直接写 ClickHouse SQL，禁止绕过 `TableRouter` 拼物理表名，禁止在没有 query pattern、压测和回归测试时提前引入重结构。
...
- 主线代码推进的节奏是“实现优先、验证跟随、评审收口”：先连续写完一个可运行的小闭环并跑相关验证，再在提交前或高风险点做审查；不要每写几行代码就启动子代理评审、外部模型评审或大段文档同步。
- 默认以“一个可验证小闭环”为节拍推进：优先连续完成 30 到 90 分钟内能落地的一组相关代码，再统一做验证、必要审查和最小文档同步；除非命中高风险边界或用户明确要求，不要把实现节奏切碎成“写一点、审一点、记一点”。
- 代码评审降频但不降质：只有在准备提交前的非平凡改动、跨模块边界改动、核心链路改动、数据契约改动、存储/队列/鉴权/支付/多租户改动、较大 UI 交互改动，或主代理对业务逻辑把握不准时，才强制启动 Codex native 子代理审查。中间迭代的小修小补、纯注释、纯文档、纯格式化、局部改名和低风险测试断言调整，可以合并到提交前统一审查。
- 代码审查必须真实完成才能在交付、提交说明或修订记录中声明“已完成代码审查”；仅启动子代理、等待超时、收到空结果、命中 `429 Too Many Requests`、命中 `402 Payment Required` / usage limit，或由主代理自行复核，都不能冒充子代理审查结论。若子代理超时、中断或因额度受限失败，必须重新发起审查或明确记录“子代理审查未完成”，不得作假。
...
- `$ai-slop-cleaner` 降到提交前关键改动或明显存在 AI 样板味、重复逻辑、弱边界时使用；不要在每次小步代码修改后机械触发。触发时仍按 bounded cleanup/review 流程：先确认行为锁定和测试证据，再按本次变更文件检查重复逻辑、死代码、无必要抽象、弱边界、样板化实现、低价值注释、缺失回归测试和无关膨胀；不能借机做大范围重写。
- `$ai-slop-cleaner` 发现必须修复的问题时，主代理先修复、重跑相关验证，并按需追加子代理复审；最终交付说明需要简要写明子代理审查结果、`$ai-slop-cleaner` 结论或剩余风险。
- 涉及 UI、页面状态、交互文案、布局、视觉层级或前端用户流程的代码修改，只有当用户可见行为发生实质变化、页面/交互风险较高或准备提交较大 UI 改动时，才需要额外完成两个外部 UI 审查；纯测试代码、端口配置、内部常量或低风险技术适配不强制触发外部 UI 审查。
- 需要外部 UI 审查时，除常规 Codex 子代理代码审查和 `$ai-slop-cleaner` 外，提交前额外完成：
