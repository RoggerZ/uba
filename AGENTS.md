...
- `analytics-core` 的 P1-002 已启动：collect 请求标准化、storage `EventWriter` 接口和 ClickHouse `TableRouter` 已落地；下一步不要绕开这些契约，真实 collect API、ClickHouse batch writer 和 Realtime/Events 查询都必须复用它们。
