...
- `analytics-core` 的 P1-002 已启动：collect 请求标准化、`collect.Handler`、fasthttp `POST /collect`、storage `EventWriter` 接口、ClickHouse `TableRouter`、native batch `BatchWriter`、GORM/MySQL `IngestionStatusGuard` 和 `EventQueryBuilder` query plan 已落地；下一步不要绕开这些契约，Redis Stream worker 装配和查询执行器都必须复用它们。
