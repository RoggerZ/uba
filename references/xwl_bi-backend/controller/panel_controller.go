...
func (this PanelController) AddReportTable(ctx *fiber.Ctx) error {
	...
	if reportTable.PannelId > 0 {
		pannelService := pannel.PannelService{}
		if err := pannelService.AttachReportTableToPannel(reportTable.PannelId, reportTable.Id, reportTable.WindowSize, c.UserID); err != nil {
			return this.Error(ctx, err)
		}
	}

	return this.Success(ctx, response.OperateSuccess, reportTable)
}
