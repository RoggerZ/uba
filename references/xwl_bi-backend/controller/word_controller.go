...
var wordLookupHTTPClient = &http.Client{Timeout: 5 * time.Second}

var loadWordDocument = func(word string) (*goquery.Document, error) {
	uri := "https://dict.youdao.com/search?q=" + url.QueryEscape(word) + "&keyfrom=new-fanyi.smartResult"
	resp, err := wordLookupHTTPClient.Get(uri)
	if err != nil {
		return nil, err
	}
	...
	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("youdao returned status %s", resp.Status)
	}
...
func GetWordParse(ctx *fasthttp.RequestCtx) {
	...
	doc, err := loadWordDocument(word.Danci)
	if err != nil {
		logs.Logger.Error("loadWordDocument", zap.Error(err))
		util.WriteJSON(ctx, map[string]interface{}{
			"code": 500,
			"msg":  "服务异常",
		})
