...
func runUserGroup(app *fiber.App) {
	...
	appG := app.Group(AbsolutePath).Use(middleware.FilterAppid)
	{
		...
		c.MountApi(api_config.MountApiBasePramas{Remark: "保存用户分群", AbsolutePath: AbsolutePath}, appG.(*fiber.Group), UserGroupController{}.SaveUserGroup)

		c.MountApi(api_config.MountApiBasePramas{Remark: "手动更新用户分群", AbsolutePath: AbsolutePath}, appG.(*fiber.Group), UserGroupController{}.RefreshUserGroup)
		...
		c.MountApi(api_config.MountApiBasePramas{Remark: "用户分群详情", AbsolutePath: AbsolutePath}, appG.(*fiber.Group), UserGroupController{}.UserGroupDetail)
