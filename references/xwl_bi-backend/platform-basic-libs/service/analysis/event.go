...
func (this *Event) getDivisorName() string {
	divisorIndex := atomic.AddInt32(&this.divisorIndex, 1)
	return "divisor" + strconv.Itoa(int(divisorIndex))
}
...
func (this *Event) GetGroupDateSql() (groupSQL string, groupCol string) {
	...
	case ByDay:
		return "  date_group ", "formatDateTime(xwl_part_date,'%Y-%m-%d') as date_group "
	case ByHour:
		return "  date_group ", " formatDateTime(xwl_part_date,'%Y-%m-%d %H:00') as date_group "
	case ByMinute:
		return "  date_group ", " formatDateTime(xwl_part_date,'%Y-%m-%d %H:%M') as date_group "
	case ByWeek:
		return "  date_group ", " concat(formatDateTime(toMonday(xwl_part_date),'%Y-%m-%d'),' ~ ',formatDateTime(addDays(toMonday(xwl_part_date), 6),'%Y-%m-%d')) as date_group "
	case Monthly:
		return "  date_group ", " formatDateTime(xwl_part_date,'%Y-%m') as date_group"
	case ByTotal:
