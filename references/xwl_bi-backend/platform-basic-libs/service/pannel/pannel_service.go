...
func (this *PannelService) RtListByAppid(appid int) (res map[int]model.ReportTable, err error) {
	...
	sql, args, err := db.
		SqlBuilder.
		Select("id,name,rt_type,data,remark").
		From("report_table").
	...
	for rows.Next() {
		...
		err := rows.Scan(&rtid, &tmp.Name, &tmp.RtType, &tmp.Data, &tmp.Remark)
		if err != nil {
			return nil, err
		}
		res[rtid] = tmp
...
func (this *PannelService) AttachReportTableToPannel(pannelId, reportTableId int, windowSize string, managerUid int32) (err error) {
	if pannelId == 0 || reportTableId == 0 {
		return nil
	}

	type pannelReportTableRow struct {
		Id           int    `db:"id"`
		ReportTables string `db:"report_tables"`
	}

	var row pannelReportTableRow
	query, args, err := db.SqlBuilder.
		Select("id,ifnull(report_tables,'') as report_tables").
		From("pannel").
		Where(db.Eq{"id": pannelId, "create_by": managerUid}).
		ToSql()
	if err != nil {
		return err
	}

	if err := db.Sqlx.Get(&row, query, args...); err != nil {
		if err == sql.ErrNoRows {
			return errors.New("target pannel not found or permission denied")
		}
		return err
	}

	nextReportTables := SerializeReportTableCards(
		AppendReportTableCard(ParseReportTableCards(row.ReportTables), reportTableId, windowSize),
	)

	_, err = db.SqlBuilder.
		Update("pannel").
		SetMap(map[string]interface{}{
			"report_tables": nextReportTables,
		}).
		Where(db.Eq{"id": pannelId, "create_by": managerUid}).
		RunWith(db.Sqlx).
		Exec()
...
FROM
	...
	list, err := userModel.Select(int64(appid))
	if err != nil {
		return
	}
...
func (this *PannelService) DeleteReportTableByID(data model.ReportTable, managerUid int32) (err error) {
	...
	var rows []pannelReportTableRow
	sql, args, err := db.SqlBuilder.
		Select("id,ifnull(report_tables,'') as report_tables").
		From("pannel").
		Where(db.Eq{"create_by": managerUid}).
		ToSql()
	if err != nil {
		return err
	}

	if err := db.Sqlx.Select(&rows, sql, args...); err != nil {
		return err
	}

	for _, row := range rows {
		nextReportTables := SerializeReportTableCards(RemoveReportTableCard(ParseReportTableCards(row.ReportTables), data.Id))
		if nextReportTables == row.ReportTables {
			continue
		}

		if _, err := db.SqlBuilder.
			Update("pannel").
			SetMap(map[string]interface{}{"report_tables": nextReportTables}).
			Where(db.Eq{"id": row.Id, "create_by": managerUid}).
			RunWith(db.Sqlx).
			Exec(); err != nil {
			return err
		}
