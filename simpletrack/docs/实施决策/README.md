...
| INFRA-001 | SimpleTrack GitHub SSH 与子仓库推送配置 | 已完成 | 已生成并记录 `id_ed25519_simpletrack` 专用 key 流程；用户交互式 PowerShell 已验证 `ssh -F config_simpletrack` 可认证到 `RoggerZ`；`analytics-core` `93cff0f`、`analytics-service` `c08e1da`、`simpletrack-saas` `ffdb254` 均已通过 `github-simpletrack` Host 与仓库级 `core.sshCommand` 推送到远端；父仓上一轮 gitlink 与文档同步 commit 为 `d1a578b` | 后续新建子仓或换机器时继续复用 `docs/Q&A/Windows-SSH仓库权限怎么配置.md` 和 `AGENTS.md` 中的专用 SSH 配置流程 |

P1.5-001 补充进度：2026-05-08 的 500k 行复测已纠正 Realtime benchmark 口径，`low_realtime_recent_window` 代表产品短窗口 Realtime，`low_realtime_wide_since` 代表宽时间窗压力查询；2026-05-09 的 `caf314d` 进一步把 Events benchmark / explain 拆成 recent-window 与 wide-window scalar/property 场景，并在执行前断言真实 `EventQueryPlan` 的时间上下界。随后 `analytics-core` commit `f84024a` 又把 typed property filters 收口到 query-builder guardrail：必须显式带 `from/to`，且 direct fact-table 查询窗口默认不超过 7 天；`analytics-core` commit `93cff0f` 再把 bounded 24h scalar Events benchmark / explain 场景收口为“只有 fixture > 86400 时才启用”，并修正精确排他上界，避免默认 10k 基线把它伪装成 wide-window；本轮继续补跑 `100000` 与 `500000` 行夹具下的 bounded 24h scalar 真实证据：两次 explain 都显示 `eligible_rows=86400`，500k 夹具下只扫 `Granules: 12/62`，benchmark 仍约 `10.0-10.6ms/op`，说明这条形状更接近 direct fact-table 的中等观察区，而不是 wide-window scalar 的 40ms 压力区；因此 `analytics-service` commit `c08e1da` 已撤回 `24h => high` 的 bounded scalar triage 规则，只保留 filter-count 主导的 bucket 语义。当前观察候选因此仍集中在“宽时间窗 scalar Events 明细查询”；如果要支持更宽的 property 历史窗口，先做方案评审而不是直接放开。当前仍不新增 ClickHouse projection、materialized view 或小时聚合表。
...
本节版本标记：`analytics-core` commit `93cff0f`；`analytics-service` commit `c08e1da`；`simpletrack-saas` commit `ffdb254`；父仓上一轮同步 commit `d1a578b`。
...
1. 继续推进 P1.5-001：围绕宽时间窗 scalar Events 明细查询继续做稳定 query pattern 和回归计划评审；typed property 过滤的 7 天窗口护栏已在 core query-builder 与 analytics-service handler 两层收口，而原先 bounded `24h` 的 `pressure=high` heuristic 已被 `c08e1da` 撤回。下一步继续用 benchmark / row-volume 证据判断是否存在更合理的 bounded scalar 阈值，或者长期保持不做时间窗特判。`docs/analytics-source-reading/read-side-benchmark-baseline.md` 已同步宽时间窗 scalar Events 仍是当前观察候选的结论。
