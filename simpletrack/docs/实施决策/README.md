...
| P1-005 | SimpleTrack Anaysitics Service 本地仓库 | 进行中 | 已创建 `src/analytics-service` 本地 Go 仓库，服务名 `simpletrack-anaysitics-service`；当前提供 `/healthz`、`/tracker.js`、`OPTIONS /collect`、`POST /collect`，并用 `MemoryResolver` 执行本地 runtime source config；`simpletrack-saas` 内部 runtime-source API 已落地 | 后续替换生产 resolver，补 Events/Realtime 查询 API，并在远端仓库创建后登记父仓 submodule |
...
- `simpletrack-anaysitics-service` 主线（本地仓库 `src/analytics-service`）：已完成本地仓库、服务骨架、memory / HTTP runtime config resolver、`/collect` 运行时校验、`/tracker.js` 静态托管、collect 单测、Redis durable enqueue、可选 ingestion worker 装配和本地/小部署 ClickHouse routed table auto migration；`simpletrack-saas` 内部 runtime-source API 已落地，下一步转向 quota、domain allowlist、internal traffic 产品配置、salt 轮换和查询 API。
...
2. 继续推进 P1-005：补 Events/Realtime 查询 API，明确 ClickHouse auto migrate 与生产迁移/回滚边界，并把 quota、domain allowlist、internal traffic 和 salt 轮换纳入后续控制面任务。
