...
| P1.5-001 | ClickHouse 读侧优化与属性治理 | 进行中 | 已完成第一步属性治理和 query plan 约束雏形：`analytics-core` 读侧查询护栏（guardrails）收口到 `readSidePolicy`，`EventQueryPlan` 开始输出结构化 `EventQueryEvidence`，并已补属性字典治理的 storage-neutral 契约、MySQL catalog adapter、`PropertyCatalogingEventWriter` 和 source-scoped `PropertyCatalogReader`；`simpletrack-anaysitics-service` 已默认把 property catalog 接入 ingestion writer 链路，并新增内部 `/v1/properties` 读回接口，复用 query token 和 write key source resolution 读取属性目录元数据；`simpletrack-saas` 已把 `/v1/properties` 接入 Events filter builder，服务端读取 source-scoped event/user property selector 并传给筛选控件作为建议，内部 query token 不暴露到浏览器，catalog selection 行为已用纯 helper 单测锁定；当 `MySQLAutoMigrate=false` 时启动期校验 `property_catalog` 表，同时把 `query_evidence` 和 `pressure` 分档透出到 readback 响应；继续保持 `EventQueryBuilder` / `EventReader` 为唯一事件读侧入口；热点明细路径再选择性引入 projection，稳定指标再落 materialized view 或小时聚合表；`pressure` 的 low / medium / high 语义已写入 Q&A 并补单测；`analytics-core` 已新增 builder-only read-side query shape benchmark、真实 ClickHouse EventReader benchmark、真实 ClickHouse BatchWriter benchmark、GORM `CreateInBatches` 对照 benchmark、Redis Stream publish / subscribe+ack benchmark、`collect.Handler` 热路径 benchmark，以及 opt-in `TestEventReaderClickHouseExplain` explain 证据测试，并新增 `docs/read-side-optimization-policy.md` 固化 projection / MV / 小时聚合表引入门槛；当前 `QueryEvidence` 已进一步补齐 effective limit、offset、time lower/upper bound、time window 和 value-free property filter shape，`simpletrack-anaysitics-service` 内部 readback API 和 OpenAPI 已透出这些字段；2026-05-08 本机真实 ClickHouse reader benchmark 已记录，10k 行下 low realtime 约 8.6-10.4ms/op、medium scalar events 约 8.6-9.7ms/op、high property events 约 15.2-16.3ms/op；100k 行 pressure run 两轮下 high property 已稳定到约 29.8-34.2ms/op，explain 进一步显示 value-free property filter shape、`CreatingSets`、3 个 `event_id in ... set` 和包含 `visit_id` 的主键条件，成为下一条重点观察候选，但当前仍以 direct fact table 为默认，事件热路径继续使用 native writer | 继续按策略文档观察 typed property 过滤读路径；如后续要直接测 catalog selection 交互，再先评审是否引入 jsdom/testing-library；没有 query evidence + benchmark + explain + 回归计划时，不新增 ClickHouse 物理结构 |
...
- `analytics-core` 已把 ClickHouse explain 证据纳入仓库级验证：`TestEventReaderClickHouseExplain` 复用 reader benchmark 的 routed table fixture 和 sealed query plan，当前 high property 场景已观测到 value-free property filter shape、`CreatingSets`、3 个 `event_id in ... set` 和包含 `visit_id` 的主键条件；这说明 typed property 过滤链路值得继续观察，但还不构成立即新增物理结构的充分依据。
- `analytics-core` 的 query plan evidence 已补齐 effective limit、offset、time lower/upper bound、bounded time window 和 value-free property filter shape；`simpletrack-anaysitics-service` 已把这些字段随 `query_evidence` 透出到内部 readback API 和 OpenAPI，避免后续从 SQL 或 URL 参数反推优化依据，同时不回显属性过滤值。
...
1. 继续推进 P1.5-001：围绕 typed property 过滤读路径继续做更大数据量、稳定 query pattern 和回归计划评审；当前两轮 100k benchmark 仍不足以触发新增 ClickHouse 物理结构。
2. 如果后续要把 Events filter builder 的 catalog selection 行为做浏览器交互级测试，先评审是否引入 jsdom/testing-library，避免为单个测试临时加重依赖。
3. 把过滤统计、salt 轮换、cookie/no-cookie、server identity、Sessions 专页和 retention 产品化维持在 P1.5/P2，除非明确重开范围。
