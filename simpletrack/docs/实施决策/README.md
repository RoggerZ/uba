...
- `analytics-core` 的 query plan evidence 已补齐 effective limit、offset、time lower/upper bound 和 bounded time window；`simpletrack-anaysitics-service` 已把这些字段随 `query_evidence` 透出到内部 readback API 和 OpenAPI，避免后续从 SQL 或 URL 参数反推优化依据。
