...
| P1-005 | SimpleTrack Anaysitics Service 本地仓库 | 进行中 | 已创建 `src/analytics-service` 本地 Go 仓库，服务名 `simpletrack-anaysitics-service`；当前提供 `/healthz`、`/tracker.js`、`OPTIONS /collect`、`POST /collect`、内部 `/v1/realtime`、内部 `/v1/events`，并用 `MemoryResolver` 或 SaaS HTTP resolver 执行 runtime source config；`simpletrack-saas` 内部 runtime-source API 与 Realtime/Events 页面服务端读回放初版已落地 | 后续补生产 resolver 形态、属性白名单来源和 query token 轮换，并在远端仓库创建后登记父仓 submodule |
...
| P1-005D | Events / Realtime 查询 API | 进行中 | `simpletrack-anaysitics-service` 已提供内部 `/v1/realtime` 与 `/v1/events` 读接口，使用 bearer token 保护并映射到 `analytics-core` 的 `EventReader`；`simpletrack-saas` Realtime / Events 页面已用 server-side readback helper 和 client-safe Website selector 接入，按组织查启用 Website 后使用 write key 调内部服务，未配置、无启用 source 或服务错误时 fail-soft；Events 现在已补 `event_name`、`distinct_id`、`limit`、`offset`、`sort_field`、`sort_direction` 白名单与 `hasMore` 分页 | 后续补属性白名单来源、query token 轮换和更复杂的查询筛选交互 |
...
- `src/analytics-service` 已补 `P1-005D` 内部读回放入口：`/v1/realtime` 和 `/v1/events` 复用 `analytics-core` 的 `EventReader`，通过内部 bearer token 保护，并支持 query `OPTIONS` preflight；Events 侧已补事件名、distinct id、排序与分页白名单。
...
2. 继续推进 P1-005：明确属性白名单来源、query token 轮换、更多查询筛选交互，补 ClickHouse auto migrate 与生产迁移/回滚边界，并把 quota、domain allowlist、internal traffic 和 salt 轮换纳入后续控制面任务。
