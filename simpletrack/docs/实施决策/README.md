...
| P0-003 | 企业分析控制台 UI 可改造性评审 | 进行中 | `src/simpletrack-saas` 已能在 Supastarter `apps/saas` 组织内导航挂载 Websites、Realtime、Events 页面草案；`simpletrack-saas` `9ebe9f4` 把 Next dev loopback 登录链路收口为共享 helper，真实验证 `localhost:3005` / `127.0.0.1:3005` 都能完成 authenticated 登录跳转；`ffdb254` 已把 Websites 页从“行内整块设置表单”重构为“紧凑来源行 + write key 面板 + 右侧动作 + accordion 设置区”，并补上“编辑成功/失败后重新展开目标来源”的上下文保持行为；`08b9acc` 又把本地 auth/mail runtime 收口为 development 默认不强制 email verification、未配置 mail provider 时仅在 development 走 console fallback，真实 curl 已验证 sign-up / sign-in 可直接进入 authenticated `/onboarding`，不再被 `EMAIL_NOT_VERIFIED` 卡住；`2a33f00` 继续把 authenticated Realtime / Events / Goals 收口成更诚实的 P1 体验：无启用 source 时统一显示 setup empty state、Realtime/Events 跳过 source-bound readback、禁用 requested source 回退到启用 source 时给出显式 notice；`075828d` 再把 setup empty state 收口为 `Websites + Quickstart` 双动作，并让 fallback notice 直达目标 source 设置折叠区；最新 `10697eb` 又补上 authenticated analytics Playwright e2e：真实覆盖 sign-up、sign-in、onboarding、org create、Realtime / Events / Goals empty state、Websites create、disabled-source fallback 和 `edit_target` 深链展开，去掉原始 Postgres 直改依赖，CI 风格 Chromium 运行也已通过 | 继续做 authenticated 页面截图级评审，并把 Quickstart/docs 跳转与更多错误态补进浏览器级回归 |
| P0-004 | Supastarter for Next.js 接入核验 | 进行中 | 已确定先选 Supastarter；`src/simpletrack-saas` 已作为独立子仓库推送；Websites 页已从 UI-only gate 前进到真实 Website source 列表 + 最小创建入口，并在 `simpletrack-saas` `ffdb254` 继续收口成更接近企业分析控制台的 accordion 行布局，同时修复 settings server action 刷新后丢上下文的问题；marketing/docs/mail-preview 已完成浏览器截图验证；privacy/terms 已从模板占位替换为 SimpleTrack 基线版本（`simpletrack-saas` `d53de11`）；authenticated readback 页面在 `simpletrack-saas` `9c8ba37` 收口了稳定 selector 和 product-safe readback copy，`9ebe9f4` 修复了 Next dev `localhost` / `127.0.0.1` 下的本地登录与 hydration 假故障，`08b9acc` 又解除了“本地无 live mail provider 时 sign-in 被 `EMAIL_NOT_VERIFIED` 卡住”的阻塞，`2a33f00` 让 Realtime / Events / Goals 在 source 未就绪或 requested source 被禁用时给出正确产品态与显式回退提示，`075828d` 继续补齐 Quickstart 次动作和统一的 Websites settings deep link helper，而最新 `10697eb` 把 Supastarter spike 补到了真实端到端产品流：测试不再依赖 raw DB mutation，Playwright 默认不再强绑本机 Chrome，`PLAYWRIGHT_PORT` 可配置且对 CI 风格运行稳定；支付先按模板已有 Stripe、Lemon Squeezy、Polar、Creem、Dodo Payments provider 接入 | 继续核验许可证、私有仓库、闭源修改、团队席位，并把 docs/quickstart 的真实跳转也纳入 authenticated 回归 |
...
本节版本标记：`analytics-core` commit `a99147f`；`analytics-service` commit `580c547`；`simpletrack-saas` commit `10697eb`；父仓同步 commit `f644227`。
