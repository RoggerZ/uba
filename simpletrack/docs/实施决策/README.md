...
| INFRA-001 | SimpleTrack GitHub SSH 与子仓库推送配置 | 进行中 | 已生成并记录 `id_ed25519_simpletrack` 专用 key 流程；用户交互式 PowerShell 已验证 `ssh -F config_simpletrack` 可认证到 `RoggerZ`；`analytics-core` `b820f7e`、`analytics-service` `f60ac59`、`simpletrack-saas` `e86ba88` 均已有远端版本；父仓正在 rebase 到远端 `7780e94` 后同步 gitlink 和文档 | 完成父仓 rebase、提交并推送，随后把父仓 commit id 回填到进度文档 |
...
本节版本标记：`analytics-core` commit `b820f7e`；`analytics-service` commit `f60ac59`；`simpletrack-saas` commit `e86ba88`；父仓本轮文档 pending commit。
...
2. 完成父仓 rebase 到远端 `7780e94` 后的文档冲突收口、提交和推送；本轮需要同步 `simpletrack-saas` gitlink `e86ba88`、Goals readback UI 状态拆分、卡壳速查索引和 DeepSeek 临时豁免记录。
3. 收口本次 Goal 最小闭环审查 `WATCH` 项：短期先记录事件名契约复制、单页最多 25 个 Goal 的 N 次 count fan-out、readback policy 控制面闭环、内部 query token 粒度和 live browser / e2e 缺口；批量 Goal count 与更窄读权限放 P1.5/P2 评审。对应版本标记见本节开头：`analytics-core` `b820f7e`，`analytics-service` `f60ac59`，`simpletrack-saas` `e86ba88`。
