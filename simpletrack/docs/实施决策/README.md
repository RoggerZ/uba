...
| P1-005E | Goal 最小闭环读回放 | 已完成 | 已完成 `analytics-core` 事件 count 读侧契约、`simpletrack-anaysitics-service` 内部 `/v1/goals`、OpenAPI、query route 配置和测试；`simpletrack-saas` 已新增 Goal 数据模型、组织内 Goals 页面、事件名校验、按 Website 列表和 24h count readback；Goal 创建会把并发唯一约束冲突映射为 `duplicate_event`，避免重复提交误报通用失败；版本标记：`analytics-core` `b820f7e`，`analytics-service` `f60ac59` + pending remote push，`simpletrack-saas` `461900a` + pending remote push，父仓 pending commit；真实代码质量复审结论为 `PASS` | 后续关注架构审查 `WATCH` 项：事件名契约共享、Goal count 批量化、readback policy 控制面闭环、更细粒度内部读权限和 live browser / e2e 缺口 |
| P1.5-001 | ClickHouse 读侧优化与属性治理 | 进行中 | 已完成属性治理和 query plan 约束雏形：`readSidePolicy`、`EventQueryEvidence`、source-scoped `PropertyCatalog`、内部 `/v1/properties`、SaaS Events filter builder 属性建议、真实 ClickHouse EventReader/BatchWriter benchmark、Redis Stream benchmark、`collect.Handler` benchmark、opt-in ClickHouse explain，以及 `QueryEvidence()` 的 `PropertyFilters` 快照语义；500k 行复测已把 Realtime 拆为短窗口 `low_realtime_recent_window` 和宽时间窗 `low_realtime_wide_since`，又把 Events 拆为 recent-window 与 wide-window scalar/property 场景；`caf314d` 已补真实 `EventQueryPlan` 时间上下界和 bound args 断言，`7c31eb2` 已把该口径同步到子仓 README 和 read-side policy，确认近期 Realtime / Events 稳定，宽时间窗 Events 与 typed property 过滤继续观察；当前仍以 direct fact table 为默认，不新增 projection、materialized view 或小时聚合表 | 继续按策略文档观察宽时间窗 Events 明细查询和 typed property 过滤读路径；没有稳定 query pattern、benchmark、explain 和回归计划时，不新增 ClickHouse 物理结构 |
...
| INFRA-001 | SimpleTrack GitHub SSH 与子仓库推送配置 | 进行中 | 已生成并记录 `id_ed25519_simpletrack` 专用 key 流程；用户交互式 PowerShell 已验证 `ssh -F config_simpletrack` 可认证到 `RoggerZ`；但 Codex 沙箱身份仍读不到 `config_simpletrack` / 私钥，当前父仓 commit `674c1de` + pending commit，子仓待推送版本为 `analytics-core` `b820f7e`、`analytics-service` `f60ac59`、`simpletrack-saas` `461900a` | 给 Codex 沙箱身份补最小读权限，或由用户在已认证 PowerShell 中推送临时 clone 后，再更新父仓 gitlink |

P1.5-001 补充进度：2026-05-08 的 500k 行复测已纠正 Realtime benchmark 口径，`low_realtime_recent_window` 代表产品短窗口 Realtime，`low_realtime_wide_since` 代表宽时间窗压力查询；2026-05-09 的 `caf314d` 进一步把 Events benchmark / explain 拆成 recent-window 与 wide-window scalar/property 场景，并在执行前断言真实 `EventQueryPlan` 的时间上下界。下一条观察候选从“泛泛 high property”收窄为“宽时间窗 Events 明细查询 + typed property 过滤读路径”。当前仍不新增 ClickHouse projection、materialized view 或小时聚合表。
...
本节版本标记：`analytics-core` commit `b820f7e`；`analytics-service` commit `f60ac59` + pending remote push；`simpletrack-saas` commit `461900a` + pending remote push；父仓本轮文档 pending commit。
...
1. 继续推进 P1.5-001：围绕宽时间窗 Events 明细查询和 typed property 过滤读路径继续做稳定 query pattern 和回归计划评审；当前 500k benchmark 证明短窗口 Realtime 稳定，但仍不足以触发新增 ClickHouse 物理结构。
2. 先在具备 `.git` 写权限的终端或修复 ACL 后提交本轮 Goal 最小闭环剩余改动；当前代码和文档已验证，但父仓和子仓 Git 元数据写入被 Windows ACL 阻止，详见 `docs/开发环境卡壳问题记录.md`。
3. 收口本次 Goal 最小闭环审查 `WATCH` 项：短期先记录事件名契约复制、单页最多 25 个 Goal 的 N 次 count fan-out、readback policy 控制面闭环、内部 query token 粒度和 live browser / e2e 缺口；批量 Goal count 与更窄读权限放 P1.5/P2 评审。对应版本标记见本节开头：`analytics-core` `b820f7e`，`analytics-service` `f60ac59` + pending remote push，`simpletrack-saas` `461900a` + pending remote push。
4. 如果后续要把 Events filter builder 的 catalog selection 行为做浏览器交互级测试，先评审是否引入 jsdom/testing-library，避免为单个测试临时加重依赖。
5. 把过滤统计、salt 轮换、cookie/no-cookie、server identity、Sessions 专页和 retention 产品化维持在 P1.5/P2，除非明确重开范围。
4. 把 R3-U1/R3-U2 的剩余项降为 P1.5/P2：ambiguous `property_indexing_status=processing` 恢复策略、ClickHouse 读侧优化中的 projection / materialized view / 小时聚合表落地。
