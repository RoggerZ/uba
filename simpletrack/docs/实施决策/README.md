...
| P1.5-001 | ClickHouse 读侧优化与属性治理 | 进行中 | 已完成第一步属性治理和 query plan 约束雏形：`analytics-core` 读侧 guardrails 收口到 `readSidePolicy`，`EventQueryPlan` 开始输出结构化 `EventQueryEvidence`，继续保持 `EventQueryBuilder` / `EventReader` 为唯一读侧入口；热点明细路径再选择性引入 projection，稳定指标再落 materialized view 或小时聚合表 | 继续补属性字典治理、query evidence 在 service / source-reading 中的使用方式，以及后续压测阈值 |
