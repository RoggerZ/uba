...
| P0-003 | 企业分析控制台 UI 可改造性评审 | 进行中 | `src/simpletrack-saas` 已能在 Supastarter `apps/saas` 组织内导航挂载 Websites、Realtime、Events 页面草案；`simpletrack-saas` `9ebe9f4` 把 Next dev loopback 登录链路收口为共享 helper，真实验证 `localhost:3005` / `127.0.0.1` 都能完成 authenticated 登录跳转；`ffdb254` 已把 Websites 页从“行内整块设置表单”重构为“紧凑来源行 + write key 面板 + 右侧动作 + accordion 设置区”，并补上“编辑成功/失败后重新展开目标来源”的上下文保持行为；`08b9acc` 又把本地 auth/mail runtime 收口为 development 默认不强制 email verification、未配置 mail provider 时仅在 development 走 console fallback；`2a33f00` 继续把 authenticated Realtime / Events / Goals 收口成更诚实的 P1 体验；`d48660e`、`aff924e`、`cb643e3`、`edc7cbd`、`246954d`、`b9a2d72`、`6a3d677`、`8eab468`、`b8a8e01` 与最新 `0760713` 继续把 Quickstart、disabled-source fallback、service outage、Websites action contract 和 authenticated readback fail-soft 纳入真实回归，其中 `0760713` 又把共享 setup state 与 Websites 创建区内联 Quickstart helper link 的新标签页语义锁进 SSR + Playwright 回归，并通过子代理二次复审收口 compile regression 与 helper-link 覆盖缺口；当前 Realtime 在 non-ready state 下不再伪装 `0/0` 指标，Events 在全 outage 下已收口为单条 warning，而 Goals 在整页共享同一 outage 时也已收口为单条 `Goals readback unavailable` warning，不再把同一条 backend error copy 逐行重复到每个 goal | 继续做 authenticated 页面截图级评审，并补更多错误态浏览器级回归 |
| P0-004 | Supastarter for Next.js 接入核验 | 进行中 | 已确定先选 Supastarter；`src/simpletrack-saas` 已作为独立子仓库推送；Websites 页已从 UI-only gate 前进到真实 Website source 列表 + 最小创建入口，并在 `ffdb254` 收口成企业控制台 accordion 行布局；marketing/docs/mail-preview 已完成浏览器截图验证；privacy/terms 已替换为 SimpleTrack 基线版本；authenticated readback 页面已完成 selector、空态、错误态、Quickstart、disabled-source fallback、service outage 和 property catalog 半失败态的真实回归，其中 `edc7cbd` 先锁住 analytics-service outage 下 Events 页仍可操作，`246954d` 补上 property suggestions fail-soft 提示，`b9a2d72` 再把同一半失败态下 Raw events 数据表仍可见的契约锁进服务端渲染回归；`6a3d677` 把 Realtime non-ready false-zero 风险和 Events 全 outage duplicate copy 收口到 SSR + Playwright 回归；`8eab468` 又把 Goals 全 outage duplicate copy 收口为共享 page-level warning + 行级通用 placeholder，并补齐 SSR / Playwright 回归；`b8a8e01` 已把 Websites 页创建成功后的 `Open Quickstart` 和 source row `Quickstart` 双 CTA 收口为真实 next-step onboarding 契约，最新 `0760713` 又继续把共享 setup state 与 Websites 创建区 helper copy 的 Quickstart 外链语义补齐到同一标准 | 继续核验许可证、私有仓库、闭源修改、团队席位，并补更多 docs/readback 错误态回归 |
...
本节版本标记：`analytics-core` commit `a99147f`；`analytics-service` commit `580c547`；`simpletrack-saas` commit `0760713`；父仓同步 commit `pending commit`。
