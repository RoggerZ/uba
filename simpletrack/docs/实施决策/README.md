...
本节版本标记：`analytics-core` commit `1ea78f3`；`analytics-service` commit `f60ac59`；`simpletrack-saas` commit `9c56e96`；父仓本轮文档 `pending commit`。
...
2. 收口 Goal 最小闭环剩余 `WATCH` 项：事件名契约复制已由 `analytics-core` `1ea78f3` 与 `simpletrack-saas` `9c56e96` 的一致样例测试和 fan-out 上限缓解，但仍非生成式共享；下一步继续评审 readback policy 控制面闭环、内部 query token 粒度和 live browser / e2e 缺口；更大规模批量 Goal count 放 P1.5/P2。
