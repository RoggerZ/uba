...
| P1.5-001 | ClickHouse 读侧优化与属性治理 | 进行中 | 已开始第一步属性治理和 query plan 约束：`analytics-core` 读侧 guardrails 收口到 `readSidePolicy`，继续保持 `EventQueryBuilder` / `EventReader` 为唯一读侧入口；热点明细路径再选择性引入 projection，稳定指标再落 materialized view 或小时聚合表 | 继续补 query plan evidence、属性字典治理和后续压测阈值 |
