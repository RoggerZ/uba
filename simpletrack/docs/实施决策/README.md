...
- `analytics-core` 已把 ClickHouse explain 证据纳入仓库级验证：`TestEventReaderClickHouseExplain` 复用 reader benchmark 的 routed table fixture 和 sealed query plan，当前 high property 场景已观测到 `CreatingSets`、3 个 `event_id in ... set` 和包含 `visit_id` 的主键条件；这说明 typed property 过滤链路值得继续观察，但还不构成立即新增物理结构的充分依据。
...
1. 继续推进 P1.5-001：围绕 typed property 过滤读路径收集 query evidence、benchmark 和 explain 证据；没有完整证据和回归计划时，不新增 ClickHouse 物理结构。
