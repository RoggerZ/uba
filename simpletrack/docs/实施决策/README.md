...
| P1-005 | SimpleTrack Anaysitics Service 本地仓库 | 进行中 | 已创建 `src/analytics-service` 本地 Go 仓库，服务名 `simpletrack-anaysitics-service`；当前提供 `/healthz`、`/tracker.js`、`OPTIONS /collect`、`POST /collect`、内部 `/v1/realtime`、内部 `/v1/events`，并用 `MemoryResolver` 或 SaaS HTTP resolver 执行 runtime source config；`simpletrack-saas` 内部 runtime-source API 已落地 | 后续补 SaaS 页面接入、生产 resolver 形态、属性白名单来源和 query token 轮换，并在远端仓库创建后登记父仓 submodule |
...
| P1-005D | Events / Realtime 查询 API | 进行中 | `simpletrack-anaysitics-service` 已提供内部 `/v1/realtime` 与 `/v1/events` 读接口，使用 bearer token 保护并映射到 `analytics-core` 的 `EventReader`；Realtime 使用短窗口，Events 使用显式时间范围和 typed sort 白名单；query routes 已补齐 `OPTIONS` preflight 以支持浏览器/SaaS 页面调用 | 后续补 SaaS 页面接入、属性白名单来源和 query token 轮换 |
...
2. 继续推进 P1-005：接入 SaaS 页面查询、明确属性白名单来源和 query token 轮换，补 ClickHouse auto migrate 与生产迁移/回滚边界，并把 quota、domain allowlist、internal traffic 和 salt 轮换纳入后续控制面任务。
