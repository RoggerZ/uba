...
| P0-004 | Supastarter for Next.js 接入核验 | 进行中 | 已确定先选 Supastarter；`src/simpletrack-saas` 已作为独立子仓库推送；Websites 页已从 UI-only gate 前进到真实 Website source 列表 + 最小创建入口；marketing/docs/mail-preview 已完成浏览器截图验证；privacy/terms 已从模板占位替换为 SimpleTrack 基线版本（`simpletrack-saas` `d53de11`）；支付先按模板已有 Stripe、Lemon Squeezy、Polar、Creem、Dodo Payments provider 接入 | 核验许可证、私有仓库、闭源修改、团队席位，并在需要 authenticated SaaS 流程时用 Docker Postgres 验证 |
...
本节版本标记：`analytics-core` commit `f84024a`；`analytics-service` commit `3b8d27c`；`simpletrack-saas` commit `d53de11`；父仓本轮文档 commit `pending commit`。
