...
| P1-002B | client info enrich 与 bot/IP 过滤 stage | 进行中 | 第一版已落地：`collect.Stage` 在 EventBus publish 前执行，`ClientEnrichmentStage` 可补 UA、referrer 和盐化 `client.ip_hash`，`TrafficFilterStage` 可按 bot UA、internal CIDR/IP 过滤；Fiber adapter 默认只用连接端 IP，需显式 `WithTrustedProxyHeaders()` 才信任代理头；浏览器 SDK 已补 `data-do-not-track="true"`，DNT active 时不发送且不持久化 `distinct_id`，并自动收集 allowlisted UTM/click id；逻辑不进入 ClickHouse writer | 后续评审 geo provider、browser/os/device 解析、internal traffic 产品配置和过滤审计策略 |
