...
| P1-002 | 数据管道最小闭环 | 进行中 | 已完成 collect 请求标准化、字段校验、`collect.Handler`、fasthttp `POST /collect` 入口、storage `EventWriter` 接口、ClickHouse `TableRouter`、native batch `BatchWriter`、`EventWriteGuard` 幂等边界、GORM/MySQL `IngestionStatusGuard`、`EventQueryBuilder` 查询边界和 `ingestion.Processor` worker 边界；子仓提交 `a22ab6e` 已推送 | 继续补真实 ClickHouse 查询执行器，并准备 Redis/MySQL/ClickHouse 本地运行依赖 |
...
- `analytics-core` P1 数据管道：collect handler、fasthttp `POST /collect` 入口、表路由契约、ClickHouse native batch writer、GORM/MySQL ingestion status guard、Realtime/Events query plan builder 和 worker 边界已完成，下一步进入真实 ClickHouse 查询执行器与本地运行依赖。
...
1. 继续实现 `analytics-core` 的真实 ClickHouse 查询执行器，让 Realtime/Events query plan 能读取结果；随后准备 Redis/MySQL/ClickHouse 本地运行依赖用于端到端验证。
