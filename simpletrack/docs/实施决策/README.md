...
| P1-005 | SimpleTrack Anaysitics Service 本地仓库 | 进行中 | 已创建 `src/analytics-service` 本地 Go 仓库，服务名 `simpletrack-anaysitics-service`；当前提供 `/healthz`、`/tracker.js`、`OPTIONS /collect`、`POST /collect`、内部 `/v1/realtime`、内部 `/v1/events`，并用 `MemoryResolver` 或 SaaS HTTP resolver 执行 runtime source config；`simpletrack-saas` 内部 runtime-source API 与 Realtime/Events 页面服务端读回放初版已落地；内部 query token 已支持短窗口轮换 allowlist，Events 属性过滤已接入 `allowed_property_filters` | 后续补生产 resolver 形态、token 生命周期治理，并在远端仓库创建后登记父仓 submodule |
...
- `src/analytics-service` 已补 `P1-005D` 内部读回放入口：`/v1/realtime` 和 `/v1/events` 复用 `analytics-core` 的 `EventReader`，通过内部 bearer token 保护，并支持 query `OPTIONS` preflight；Events 侧已补事件名、distinct id、排序、分页和 typed property filters 白名单；内部 query token 已支持短窗口轮换 allowlist。
...
- `simpletrack-anaysitics-service` 主线（本地仓库 `src/analytics-service`）：已完成本地仓库、服务骨架、memory / HTTP runtime config resolver、`/collect` 运行时校验、`/tracker.js` 静态托管、collect 单测、Redis durable enqueue、可选 ingestion worker 装配、本地/小部署 ClickHouse routed table auto migration，以及 `P1-005D` 内部 Events / Realtime 查询入口、query token 轮换 allowlist 和 source-scoped 属性过滤白名单；`simpletrack-saas` 内部 runtime-source API、Realtime/Events 页面读回放和 client-safe Website selector 初版已落地，下一步转向 quota、domain allowlist、internal traffic 产品配置、salt 轮换、token 生命周期治理和更复杂筛选交互。
...
2. 继续推进 P1-005：明确 query token 生命周期/审计、更多查询筛选交互，补 ClickHouse auto migrate 与生产迁移/回滚边界，并把 quota、domain allowlist、internal traffic 和 salt 轮换纳入后续控制面任务。
