...
2. 继续推进 P1-005：补更多查询筛选交互，补 ClickHouse auto migrate 与生产迁移/回滚边界，并把更完整的 quota、internal traffic 和 salt 轮换纳入后续控制面任务。
