...
本节版本标记：`analytics-core` commit `f84024a`；`analytics-service` commit `0248a80`；`simpletrack-saas` commit `e69cea6`；父仓本轮文档 commit 待回填。
...
1. 继续推进 P1.5-001：围绕宽时间窗 scalar Events 明细查询和 7 天内 typed property 过滤读路径继续做稳定 query pattern 和回归计划评审；当前 500k benchmark 证明短窗口 Realtime 稳定，而更宽的 property 历史窗口已先被 query-builder guardrail 收口，仍不足以触发新增 ClickHouse 物理结构。
