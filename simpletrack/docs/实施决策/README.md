...
1. 继续评审 P1-002C 剩余项：`visit_id` 持久化方案已定，browser / OS / device、geo enrich、internal traffic 产品配置和过滤审计边界已落地；过滤统计、salt 轮换、cookie/no-cookie、server identity、Sessions 专页和 retention 产品化放 P1.5/P2。
2. 继续推进 P1-005：在组合查询已打通的基础上，继续补更复杂的聚合查询交互，保留 ClickHouse 本地/小部署建表与启动校验边界；生产迁移/回滚逻辑等真实上线后再单独评审；quota 当前只执行 active website source 上限，后续重点转向 salt 轮换和更复杂查询组合。
