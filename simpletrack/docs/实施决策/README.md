...
| 日期 | 修订内容 | 影响范围 |
| --- | --- | --- |
| 2026-05-03 | `simpletrack-anaysitics-service` 增加 `ANALYTICS_SERVICE_SOURCE_RESOLVER=http` 控制面 HTTP resolver，可用 bearer token 向 SaaS 控制面读取 runtime source config，并用短 TTL 缓存降低 collect 热路径控制面压力；控制面 URL 默认必须是 HTTPS，本地 loopback HTTP 需要显式开启 `ANALYTICS_SERVICE_CONTROL_PLANE_ALLOW_INSECURE_LOOPBACK=true`；同进程 ingestion 仍要求 `ANALYTICS_SERVICE_SOURCES_JSON` 作为启动 schema surface，并拒绝 HTTP resolver 返回未在启动阶段校验过的 source | P1-005C、simpletrack-anaysitics-service、SaaS control-plane runtime enforcement |
...
| P1-005 | SimpleTrack Anaysitics Service 本地仓库 | 进行中 | 已创建 `src/analytics-service` 本地 Go 仓库，服务名 `simpletrack-anaysitics-service`；当前提供 `/healthz`、`/tracker.js`、`OPTIONS /collect`、`POST /collect`，并用 `MemoryResolver` 执行本地 runtime source config | 后续替换生产 resolver，补 Events/Realtime 查询 API，并在远端仓库创建后登记父仓 submodule |
...
- `simpletrack-anaysitics-service` 主线（本地仓库 `src/analytics-service`）：已完成本地仓库、服务骨架、memory / HTTP runtime config resolver、`/collect` 运行时校验、`/tracker.js` 静态托管、collect 单测、Redis durable enqueue、可选 ingestion worker 装配和本地/小部署 ClickHouse routed table auto migration；下一步补 SaaS 内部 runtime-source API、部署级 schema migration/rollback 规范和查询 API。
...
2. 继续推进 P1-005：在 `simpletrack-saas` 落地内部 runtime-source API，明确 ClickHouse auto migrate 与生产迁移/回滚边界，并设计 Events/Realtime 查询 API。
