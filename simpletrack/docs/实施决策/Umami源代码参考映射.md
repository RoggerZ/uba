...
- `simpletrack-analytics-service` 承接 SimpleTrack 运行时数据面：write key、Origin/CORS、internal traffic、bot 过滤、tracker 静态交付和调用 `analytics-core`。
...
| tracker auto pageview、custom event、identify | P1-004、P1-005 | P1 已吸收为无依赖浏览器 SDK 和 docs/quickstart，并补齐 opt-in DNT 和 allowlisted UTM/click id；SDK 已从 `analytics-core` 迁出，由 `simpletrack-analytics-service` 静态托管，并改为 `data-write-key` 接入；多框架 SDK、CDN 版本化和 performance metrics 后置 | Browser SDK -> `simpletrack-analytics-service` `/collect` -> `analytics-core/collect.Request` 协议 |
...
1. P1-002E 端到端验证已复用现有 `analytics-core` 契约完成，没有绕回 Next.js API route 写库模式；`analytics-core` 已进一步纠偏为 Go library，SimpleTrack 运行时入口进入 `simpletrack-analytics-service`。
