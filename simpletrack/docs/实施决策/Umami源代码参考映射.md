...
- P1-005C 已补 HTTP source resolver 作为 SaaS 控制面读取 runtime source config 的生产接入雏形；它只读配置，不创建或修改站点/write key/domain/quota；默认要求 HTTPS，本地 loopback HTTP 必须显式 opt-in，同进程 ingestion 会把动态返回 source 限定在启动 schema surface 内。
