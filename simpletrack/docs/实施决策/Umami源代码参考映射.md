...
| source + id 或 IP/UA/salt 派生 session/visit | P1-002C | 已吸收为盐化窗口 session resolver 和持久化 visit resolver：原始 IP/UA 不落库；浏览器 DNT opt-in 已避免本地持久 identity；`visit_id` 已进入写入契约、ClickHouse event / `_properties` 表和读侧查询 | `SessionResolverStage`、`VisitResolverStage`，salt 轮换、cookie/no-cookie、Sessions 页面和 retention 产品化后置 |
| `FILTER_COLUMNS`、operator mapping、分页 | P1-002D、P1-005D | 已吸收为 Events 排序/过滤字段白名单、operator enum、filter 数量上限、typed property filter allowlist 和非法 filter 返回规范；现在 SaaS Events 页面也把 `event_name`、`distinct_id`、`visit_id`、`sort_field`、`sort_direction`、`limit`、`offset` 做服务端归一化后再读回放；属性过滤已通过真实 ClickHouse e2e，并在 `simpletrack-anaysitics-service` 通过 source runtime config `allowed_property_filters` 暴露内部查询入口；query token 轮换已支持结构化生命周期和审计日志 | `EventQueryBuilder` 强契约和测试 + SaaS readback 归一化 |
...
2. P1-005D 已把 Events 的服务端筛选、排序、分页归一化、属性过滤入口、持久化 `visit_id` 读取和 query token 短窗口轮换落在 SaaS / `simpletrack-anaysitics-service` readback 边界；当前还补了 token 生命周期元数据和审计日志；后续在 `analytics-core` 自定义属性、client enrich 和 query builder 继续推进时，对照 Umami 的 `event_data`、`session_data`、`FILTER_COLUMNS`、operator mapping 和 ClickHouse schema。
3. R3-U1 的 P1 写入组合已完成；P1-002B/C 已落地到 collect pre-queue stage，`visit_id` 持久化方案已定；P1-005B 已补本地/小部署 routed table auto migration。R3-U2 和 ClickHouse 属性治理、去重、聚合优化进入 P1.5/P2 评审，geo/DNT 等剩余项继续按 R3-U3 到 R3-U5 跟进。
