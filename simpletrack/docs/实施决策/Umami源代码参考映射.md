...
2. P1-005D 已把 Events 的服务端筛选、排序、分页归一化、属性过滤入口和 query token 短窗口轮换落在 SaaS / `simpletrack-anaysitics-service` readback 边界；当前还补了 token 生命周期元数据和审计日志；后续在 `analytics-core` 自定义属性、session/visit、client enrich 和 query builder 继续推进时，对照 Umami 的 `event_data`、`session_data`、`FILTER_COLUMNS`、operator mapping 和 ClickHouse schema。
