...
| bot/IP/internal traffic 过滤 | P1-002B | 第一版已吸收为 bot UA 与 internal CIDR/IP pre-queue 过滤；产品 UI、allow/deny 配置、DNT、audit 继续评审 | `TrafficFilterStage` + `FilteredError`，不进入 ClickHouse writer |
| source + id 或 IP/UA/salt 派生 session/visit | P1-002C | 第一版已吸收为盐化窗口 session resolver，原始 IP/UA 不落库；`visit_id`、salt 轮换、cookie/no-cookie、DNT 和 retention 继续评审 | `SessionResolverStage`，后续再决定 `visit_id` 契约 |
...
| tracker auto pageview、custom event、identify | P1-004 | P1 已吸收为无依赖浏览器 SDK 和 docs/quickstart；多框架 SDK、CDN 版本化和 performance metrics 后置 | `analytics-core/sdk/browser/tracker.js` -> `collect.Request` 协议 |
