...
| --- | --- | --- | --- |
| `event_data` / `session_data` typed 属性表 | P1-002A | 已吸收为 collect 属性入口约束、storage typed row 逻辑展开和独立 ClickHouse property writer；property writer 是否并入 ingestion 热路径、属性字典、属性过滤仍待评审 | `EventPropertyRecord` / `FlattenEventProperties`、`EventPropertyWriter` / `PropertyBatchWriter`、后续属性字典、`EventQueryBuilder` 属性过滤 |
