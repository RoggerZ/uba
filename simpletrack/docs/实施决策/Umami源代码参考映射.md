...
| --- | --- | --- | --- |
| `event_data` / `session_data` typed 属性表 | P1-002A | 已吸收为 collect 属性入口约束和 storage typed row 逻辑展开；ClickHouse typed rows、Map/JSON、原始 JSON + 高频属性展开的物理存储取舍仍待评审 | `EventPropertyRecord` / `FlattenEventProperties`、后续 `EventWriter` 属性写入、属性字典、`EventQueryBuilder` 属性过滤 |
