...
1. `collect` 接收事件。
2. `EventBus` 抽象队列。
3. Kafka-first `EventBus` provider 作为生产主实现。
4. `RedisStreamBus` 保留为本地、小流量和测试实现。
...
- **低运维起步**：本地默认仍可使用 Redis Stream，降低早期启动复杂度；生产语义以 Kafka provider 为主。
...
- 不把 Kafka 作为本地默认或唯一运行依赖；Redis Stream 仍可用于本地、小流量和测试。
...
| Kafka 与采集链路耦合 | 高 | 当前 producer/sinker 均围绕 Kafka topic 和 Sarama 实现 | P1 已抽象 `EventBus`，Kafka provider 作为生产主实现，Redis Stream 保留为本地/小流量/测试 provider |
...
- `KafkaBus`：生产主实现，使用 Sarama 发布 JSON `EventEnvelope`，consumer group 消费，provider 内部负责 retry、DLQ、completion gate 和 per-partition ordered commit；`simpletrack-anaysitics-service` 通过默认关闭的内部 diagnostics route 读取 provider 进程级 `Stats()` 快照，并通过默认关闭的 metrics route 输出 Prometheus text 快照。
- `RedisStreamBus`：本地、小流量和测试实现，继续用 pending 优先重试和死信 stream 模拟同类语义。
- `DirectBus`：本地开发、单进程测试、最小 demo，不允许作为生产 ingestion provider。

公共 `eventbus.Message` 语义：

- `Message.ID` 是 provider-native delivery id，用于队列排障和投递诊断；Kafka 形如 `topic:partition:offset`，Redis Stream 使用 stream entry id，Direct 使用 `direct:N`。
- `Message.Envelope.ID` 是业务事件 id，ingestion 幂等、ClickHouse `event_id`、产品读回放和去重都必须使用它。
- handler 不允许通过 `Message.ID` 推断事件身份，也不直接 ack/nack；返回 `error` 后由 provider 决定 retry、DLQ 或 ordered commit。
...
### Step 3：Kafka-first EventBus、Redis Stream 与 direct

交付：

- 定义 `EventBus` 接口。
- 公共 handler 只返回 error，不直接 ack/nack；provider 内部负责 retry、DLQ 和 commit。
- 实现 `KafkaBus`、`RedisStreamBus` 和 `DirectBus`。
- Kafka provider 使用 `github.com/IBM/sarama`，Sarama session / claim / MarkMessage 只留在 `eventbus/kafka` 内部。
- 本地 Redis 使用 `redis/redis-stack:latest` 容器镜像；本地 Kafka 使用单节点 `bitnami/kafka:3.9`。
- Kafka 消费采用 per-partition ordered commit：后面的 offset 即使先完成，也必须等待前面的 offset 完成或进入 DLQ 后才能推进连续提交区间。
- Kafka DLQ 写成功后才完成原消息；DLQ 写失败时原 offset 不提交，避免失败消息既没入库也没进 DLQ。
...
- Kafka provider 单测覆盖 publish、retry、DLQ 成功/失败、ordered commit、stale generation、completion gate、worker pool panic/close/cancel；Kafka live integration test 默认跳过，可用本地 Kafka env 显式开启。
- Redis Stream 与 KafkaBus 使用同一事件协议，差异留在 provider 内部。
...
| Kafka provider | `KafkaBus` 已实现 Sarama producer / consumer group / ordered commit / completion gate / retry / DLQ；生产默认已收口到 `acks=all` / `WaitForAll` 与 retry 默认值；当前又补齐 rebalance-safe generation abort、诊断 `Stats()`、TLS/mTLS、SASL/PLAIN、ACL/runbook、KafkaBus provider benchmark、真实 broker publish benchmark、replicated topic integration、disposable broker outage gate、本机 disposable `SASL_SSL` broker 认证 integration、3 broker KRaft `SASL_SSL` 生产形态 drill、同一认证环境下的真实 broker publish benchmark，以及 `analytics-service` `c6d1139` 的默认关闭内部 `/v1/kafka/diagnostics` JSON route 和 `/v1/kafka/metrics` Prometheus text route；父仓 `848494a` 已完成 broker/exporter SLO 指标源分层，明确 broker lag / ISR / partition / request 指标不由 service 进程内 metrics 替代；KAFKA-PROD-008 路线评审已把 SCRAM-SHA-512/256 标为下一步高级认证先做候选，OAuthBearer 暂缓，Kerberos/GSSAPI 当前否决、仅企业 Kerberos 体系触发；当前 runtime 仍只声明 `SASL/PLAIN + TLS` 已支持 |
...
| Redis Stream 消费 | pending 优先重试，写入成功才 ack，超过 `MaxAttempts` 后进入死信队列；定位为本地、小流量和测试 provider |
...
| 压测基线 | 需要建立 `analytics-core` 独立压测基线，覆盖 collect、Redis Stream、KafkaBus、ClickHouse 写入和典型查询；当前已补 builder-only read-side shape benchmark、真实 ClickHouse EventReader benchmark、真实 ClickHouse BatchWriter benchmark、GORM `CreateInBatches` 对照 benchmark、Redis Stream publish / subscribe+ack benchmark、`collect.Handler` 热路径 benchmark、KafkaBus provider 内部 benchmark 和真实 broker publish benchmark 入口；collect 本地基线约为 normalize+publish 3.2-6.1µs/op、identity resolver 11.0-12.0µs/op、identity + client enrichment 15.2-18.2µs/op；ClickHouse 单事件 native writer 约 5.2-6.3ms/op，GORM 单事件约 7.3-8.2ms/op；100 行 native bulk 约 6.3-8.2ms/op，GORM bulk 约 8.0-10.1ms/op；KafkaBus provider 内部本机 200ms 基线约 publish 902.9ns/op、process no-op 2119ns/op、ordered committer register/complete 128.1ns/op；当前单 broker plaintext 真实 publish benchmark 约 `1.03-1.33ms/op`、`7011-7214 B/op`、`78 allocs/op`；本机 3 broker `SASL_SSL` 认证 publish benchmark 默认自动校准波动约 `275.2ms/op`、`293.7ms/op`、`1005.9ms/op`，固定 `10x` 复测约 `108.9-117.3ms/op`、`82.3-111.5KB/op`、`342-442 allocs/op`，只作为本机 Docker Desktop + TLS/SASL + SyncProducer 相对基线；后续继续补更大规模 broker/partition/message-size 和稳定聚合查询压测 |
...
- Kafka provider 的下一层生产硬化：本轮已完成 `acks=all` / `WaitForAll` 默认、idempotent producer opt-in 约束、SASL/TLS 与 broker ACL 配置矩阵、consumer lag / DLQ / retry / pause-resume 诊断快照、`analytics-service` 内部 diagnostics route、内部 metrics route、rebalance ordered commit 场景、kill/restart replay 验证、replicated topic integration、disposable broker outage gate、KafkaBus provider 内部 benchmark、真实 broker publish benchmark 入口、本机 disposable 单 broker `SASL_SSL` 认证 integration、3 broker KRaft `SASL_SSL` drill、同环境认证 broker benchmark、broker/exporter SLO 指标源分层，以及 SCRAM / OAuth / Kerberos 路线评审；后续如果目标 Kafka 确认需要高级认证，优先把 SCRAM-SHA-512/256 拆成代码任务，OAuthBearer 暂缓，Kerberos/GSSAPI 仅企业定制触发；dashboard / alert rules 和生产阈值仍另开任务。
