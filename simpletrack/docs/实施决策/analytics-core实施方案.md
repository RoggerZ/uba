...
- Kafka provider 的下一层生产硬化：本轮已完成 `acks=all` / `WaitForAll` 默认、idempotent producer opt-in 约束、SASL/TLS 与 broker ACL 配置矩阵、consumer lag / DLQ / retry / pause-resume 诊断快照、`analytics-service` 内部 diagnostics route、内部 metrics route、rebalance ordered commit 场景、kill/restart replay 验证、replicated topic integration、disposable broker outage gate、KafkaBus provider 内部 benchmark 和真实 broker publish benchmark 入口；后续只评审 SCRAM/OAuth/Kerberos、真实认证 broker live 证据、生产 SLO 阈值和更大规模真实吞吐压测阈值。
