...
| Kafka provider | `KafkaBus` 已实现 Sarama producer / consumer group / ordered commit / completion gate / retry / DLQ；SASL/TLS、producer idempotence、acks 策略和 rebalance 集成压测后续评审 |
...
- Kafka provider 的生产硬化：明确 `acks=all` / `WaitForAll` 是否成为生产默认、是否开启 idempotent producer、SASL/TLS 与 broker ACL 配置矩阵、consumer lag / DLQ / retry / pause-resume observability、rebalance 集成场景、kill/restart replay 验证和 KafkaBus 压测阈值。
