...
6. P1 数据闭环稳定后，P1.5-001 先做属性治理和 query plan 约束；当前已补 `readSidePolicy`、`EventQueryEvidence`、`PropertyCatalog` 基础契约、MySQL catalog adapter、`PropertyCatalogingEventWriter` 和 `simpletrack-anaysitics-service` ingestion 运行时装配；`simpletrack-anaysitics-service` 的 readback 响应已开始透出 `query_evidence` 与 `pressure`，其中 `pressure` 只是 low / medium / high 的 triage 桶；`analytics-core` 已新增 builder-only read-side shape benchmark，先固定 low realtime、medium scalar events、high property events 三类查询的 plan 构建基线；projection、materialized view 和小时聚合表只在真实 ClickHouse 执行压测、热点路径和稳定指标口径明确后逐步引入。
...
| 压测基线 | 需要建立 `analytics-core` 独立压测基线，覆盖 collect、Redis Stream、KafkaBus、ClickHouse 写入和典型查询；当前已先补 builder-only read-side shape benchmark，后续补真实 ClickHouse 执行压测 |
