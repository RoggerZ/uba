...
Events 是明确时间范围内的分页列表，必须传 `from` 和 `to`，支持 `event_name`、`distinct_id`、`visit_id`、排序、分页和 property filter。当前 `analytics-core` commit `f84024a` 还把 typed property filter 收口到 query-builder guardrail：带属性过滤的 Events 查询必须显式带 `from/to`，并且 direct fact-table 路径默认只允许 7 天内窗口。证据：`仓库: analytics-service, commit: 09656b6, file: internal/collectapi/query.go:140-191`
