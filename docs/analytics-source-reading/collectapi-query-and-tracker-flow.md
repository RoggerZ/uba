...
>
> 重点入口：
> - `writeTracker`：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/handler.go:318-325`
> - `handleRealtime`：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:72-120`
> - `handleEvents`：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:122-192`
...
| --- | --- | --- |
| `src/analytics-service` | `825e366c014b25e7e0198f56f9d683e046d458ab` | `git status --short` 仅显示未跟踪 `.idea/`，本文引用的源码文件与 `HEAD` 一致 |
...
`NewApp` 创建 Fiber app，安装 CORS middleware，然后调用 `registerRoutes`。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/handler.go:61-82`
...
`registerRoutes` 把 `TrackerPath`、`RealtimePath`、`EventsPath` 分别绑定到三个 handler。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/handler.go:153-158`
...
默认路径来自配置层：`/tracker.js`、`/v1/events`、`/v1/realtime`。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/config/config.go:14-23`
...
`runtime.New` 是装配层：读取 tracker 文件，创建 query reader，再把依赖放进 `collectapi.Options`。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/runtime/runtime.go:49-85`
...
`newQueryReader` 只在 query enabled 时打开 ClickHouse 读连接，创建 `TableRouter`、`EventQueryBuilder`、`clickhouse.EventReader`。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/runtime/runtime.go:156-187`
...
`newHandler` 默认把 `TrackerPath` 设为 `/tracker.js`。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/handler.go:85-100`
...
`registerRoutes` 绑定 `GET /tracker.js -> writeTracker`。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/handler.go:153-155`
...
配置默认 tracker 文件是 `public/tracker.js`。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/config/config.go:14-23`

```go
defaultTrackerFile = "public/tracker.js"
```

启动装配时 `runtime.New` 用 `os.ReadFile(cfg.TrackerFile)` 读入字节数组，再放进 `collectapi.Options.TrackerScript`。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/runtime/runtime.go:49-55`、`仓库: analytics-service, commit: 825e366c014b, file: internal/runtime/runtime.go:68-85`
...
tracker 文件自身会从 script tag 读取 `data-write-key`、`data-collect-url` 等配置。证据：`仓库: analytics-service, commit: 825e366c014b, file: public/tracker.js:33-46`
...
它发送事件时把 `write_key` 放进 JSON body，并 POST 到 collect URL。证据：`仓库: analytics-service, commit: 825e366c014b, file: public/tracker.js:263-304`
...
证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/handler.go:318-325`
...
如果 `TrackerScript` 为空，返回 404 JSON。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/handler.go:318-321`
...
另外，正常 `runtime.New` 会在启动时读取 tracker 文件；文件不存在会直接返回错误，不进入服务监听。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/runtime/runtime.go:49-55`

### 3.2 handleRealtime

#### route 是如何注册的

默认 path 是 `/v1/realtime`，也可由 `ANALYTICS_SERVICE_REALTIME_PATH` 覆盖。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/config/config.go:19-20`、`仓库: analytics-service, commit: 825e366c014b, file: internal/config/config.go:97-98`
...
注册点：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/handler.go:153-158`
...
`handleRealtime` 第一层检查 `QueryReader`，没有配置时直接 404。然后调用 `requireQueryToken`。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:72-81`
...
`requireQueryToken` 从 `Authorization` header 提取 Bearer token，调用 `authorizeQueryToken`。未知、过期、未生效都返回 401。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:194-210`
...
`authorizeQueryToken` 用常量时间比较遍历所有凭证，并判断 `NotBefore` / `ExpiresAt`。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query_auth.go:69-98`
...
证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:261-266`
...
`resolveQuerySource` 会先取 write key，空值返回 400；再调用 `h.opts.Resolver.ResolveSource`。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:212-225`
...
`SourceConfig` 是控制面给 analytics-service 的运行时 source 档案。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/controlplane/resolver.go:18-37`
...
读接口不是“覆盖请求体字段”，而是根本不接受客户端传 tenant/project/source。`handleRealtime` 用 resolver 返回的 `source.TenantID`、`source.ProjectID`、`source.SourceID` 构造 `storage.RealtimeQuery`。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:101-109`
...
`since` 默认是当前时间减 30 分钟，`limit` 默认 50 且必须大于等于 1。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:17-22`、`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:90-99`
...
时间格式只接受 RFC3339 / RFC3339Nano。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:317-327`
...
返回结构是 `source + items + since + limit`。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:57-62`、`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:114-119`
...
`EventRecord` 会被映射成 JSON 字段，包括 `visit_id`、`properties`、`user_properties`。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:482-498`
...
| 失败情况 | 证据 | HTTP |
| --- | --- | --- |
| query reader 未配置 | `仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:72-77` | `404 {"error":"not found"}` |
| query token 缺失/未知/过期/未生效 | `仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:194-210` | `401 {"error":"unauthorized"}` |
| write key 缺失 | `仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:216-220` | `400 {"error":"write_key is required"}` |
| write key 无效 | `仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/handler.go:306-315` | `401 {"error":"invalid write key"}` |
| source disabled | `仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/handler.go:306-315` | `403 {"error":"source is disabled"}` |
| origin 不允许 | `仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:226-230` | `403 {"error":"origin is not allowed"}` |
| query 参数非法 | `仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:92-99` | `400 {"error":"..."}` |
| reader 返回 `ErrInvalidEventQuery` | `仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:268-275` | `400 {"error":"invalid event query: ..."}` |
| reader 其他错误 | `仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:268-275` | `500 {"error":"internal server error"}` |

### 3.3 handleEvents

#### route 是如何注册的

默认 path 是 `/v1/events`，也可由 `ANALYTICS_SERVICE_EVENTS_PATH` 覆盖。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/config/config.go:19-20`、`仓库: analytics-service, commit: 825e366c014b, file: internal/config/config.go:97-98`
...
与 `handleRealtime` 相同：先要求 `QueryReader`，再要求 query token。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:122-130`
...
query token 来源和生命周期来自环境变量。`LoadFromEnv` 读取 `ANALYTICS_SERVICE_QUERY_TOKEN`，并解析可选 JSON rotation list。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/config/config.go:135-143`、`仓库: analytics-service, commit: 825e366c014b, file: internal/config/config.go:218-271`
...
主 token 可配置 `ANALYTICS_SERVICE_QUERY_TOKEN_ID`、`ANALYTICS_SERVICE_QUERY_TOKEN_NOT_BEFORE`、`ANALYTICS_SERVICE_QUERY_TOKEN_EXPIRES_AT`。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/config/config.go:274-289`
...
轮换 token 可是字符串，也可以是带 `id`、`token`、`not_before`、`expires_at` 的对象。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/config/config.go:291-327`

#### write key 从哪里来

与 Realtime 相同：`X-SimpleTrack-Write-Key` header 优先，否则 `write_key` query。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:261-266`
...
| 参数 | 是否必填 | 解析位置 | 说明 |
| --- | --- | --- | --- |
| `write_key` | 是 | `仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:212-220` | 也可用 `X-SimpleTrack-Write-Key` header |
| `from` | 是 | `仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:140-145` | RFC3339 / RFC3339Nano，包含起点 |
| `to` | 是 | `仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:146-149` | RFC3339 / RFC3339Nano，不含终点 |
| `limit` | 否 | `仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:150-153` | 默认 100，HTTP 层要求整数且至少 1 |
| `offset` | 否 | `仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:154-157` | 默认 0，HTTP 层要求整数且至少 0 |
| `event_name` | 否 | `仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:165-179` | trim 后进入 `EventListQuery.EventName` |
| `distinct_id` | 否 | `仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:165-179` | trim 后进入 `EventListQuery.DistinctID` |
| `sort_field` | 否 | `仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:165-179` | 传给 analytics-core 白名单校验 |
| `sort_direction` | 否 | `仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:165-179` | 传给 analytics-core 白名单校验 |
| `property_filter` | 否，可重复 | `仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:329-402` | 每个值是 URL 解码后的 JSON |
...
证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:294-315`、`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:329-349`
...
`SourceConfig` 有 `AllowedPropertyFilters` 字段。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/controlplane/resolver.go:18-43`

```go
AllowedPropertyFilters []AllowedPropertyFilter `json:"allowed_property_filters"`
```

每个 `property_filter` 先被解析成 JSON：`scope`、`name`、`type`、`op`、`value`。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:64-70`
...
然后必须命中 source allowlist，否则直接返回 `ErrInvalidEventQuery`。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:364-402`
...
`AllowsPropertyFilter` 会按 scope/name 匹配，并可按 value type 限制。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/controlplane/resolver.go:80-108`
...
最后 HTTP 层还把 selector surface 传给 analytics-core builder，形成第二道 fail-closed guard。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:165-179`、`仓库: analytics-core, commit: 58668c9c17ea, file: storage/clickhouse/query_builder.go:313-332`
...
`handleEvents` 把解析后的参数装进 `storage.EventListQuery`，调用 `ListEvents`。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:163-181`
...
返回结构是 `source + items + limit + offset + from + to`。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:48-55`、`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:184-191`
...
OpenAPI 同样定义了 `EventsResponse` 必须包含这些字段。证据：`仓库: analytics-service, commit: 825e366c014b, file: api/openapi.yaml:352-370`
...
证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:140-160`、`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:268-275`、`仓库: analytics-core, commit: 58668c9c17ea, file: storage/clickhouse/query_builder.go:144-206`
...
对应代码证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/handler.go:318-325`
...
响应结构证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:31-62`
...
响应结构证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:48-55`
...
Go 侧提取 Bearer token 的函数是通用 helper。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/handler.go:386-392`
...
生命周期字段定义在配置层和 collectapi 层。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/config/config.go:32-38`、`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query_auth.go:10-16`
...
证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query_auth.go:33-67`、`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query_auth.go:69-98`、`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:235-258`
...
HTTP 层再把 `Properties` 字符串转成 `json.RawMessage`。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:501-514`
...
如果 source resolver 是 HTTP 模式，analytics-service 会 POST write key 到 SaaS runtime-source API。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/controlplane/http_resolver.go:194-238`
...
| 项 | 内容 |
| --- | --- |
| 定义位置 | `仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/handler.go:23-42` |
| 类型 | `[]byte` |
...
| 项 | 内容 |
| --- | --- |
| 定义位置 | `仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/handler.go:23-42`、`仓库: analytics-core, commit: 58668c9c17ea, file: storage/event_query.go:140-146` |
...
| 项 | 内容 |
| --- | --- |
| 定义位置 | `仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query_auth.go:10-16` |
| 类型 | `QueryCredential` |
...
| 项 | 内容 |
| --- | --- |
| 定义位置 | `仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:194-210`、`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/handler.go:386-392` |
...
| 项 | 内容 |
| --- | --- |
| 定义位置 | `仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:261-266` |
| 类型 | `string` |
...
| 项 | 内容 |
| --- | --- |
| 定义位置 | `仓库: analytics-service, commit: 825e366c014b, file: internal/controlplane/resolver.go:18-37` |
...
| 项 | 内容 |
| --- | --- |
| 定义位置 | `仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:64-70`、`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:329-402` |
...
| 项 | 内容 |
| --- | --- |
| 定义位置 | `仓库: analytics-service, commit: 825e366c014b, file: internal/controlplane/resolver.go:39-43` |
...
| 项 | 内容 |
| --- | --- |
| 定义位置 | `仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:31-62` |
...
| 项 | 内容 |
| --- | --- |
| 定义位置 | `仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/handler.go:56-59` |
| 类型 | `ErrorResponse` |
| 用途 | 所有稳定错误响应都用 `{"error":"..."}` |
...
| --- | --- | --- | --- | --- |
| route registration | `Options.*Path` | Fiber route table | path 为空、不是 `/` 开头、冲突时启动失败 | `TrackerPath` / `RealtimePath` / `EventsPath` 绑定 handler。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/handler.go:126-169` |
| tracker asset response | `TrackerScript` | JS bytes response | script 为空返回 404 | 启动时文件 bytes 变成 HTTP JS 响应。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/handler.go:318-325` |
| query auth validation | `Authorization`、`QueryCredentials` | `queryTokenAuthDecision` | 401 unauthorized | Bearer token 被常量时间比较，并检查生命周期。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query_auth.go:69-98` |
| write key extraction | header/query | `writeKey` | 空值返回 400 | 读接口不接受 body fallback。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:261-266` |
| source resolution | `writeKey` | `SourceConfig` | invalid key 401、disabled 403、resolver error 500 | write key 变成可信 scope。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:212-233` |
| source enabled / origin / scope enforcement | `SourceConfig`、`Origin` | scoped query | origin 403 | tenant/project/source 从 SourceConfig 进入 query object。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:226-230` |
| query parameter parsing | HTTP query | `since/from/to/limit/offset/sort` | 400 | 时间转 UTC，数字做 min 限制。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:278-327` |
| property filter parsing and allowlist enforcement | repeatable `property_filter`、SourceConfig allowlist | `[]storage.EventPropertyFilter` | 400 invalid event query | JSON 标量转 typed property filter，先过 SourceConfig allowlist。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:329-402` |
| query reader call | `RealtimeQuery` / `EventListQuery` | `[]EventRecord` | invalid query 400，其他 500 | HTTP 层调用接口，不拼 SQL。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:101-119`、`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:163-191` |
| response serialization | `[]EventRecord` | JSON body | JSON 写入失败交给 Fiber error handler | 时间格式化，properties 尽量保持 JSON。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:474-514` |
| error mapping | Go error | HTTP status + `ErrorResponse` | 无 | `ErrInvalidEventQuery` 映射 400，其余 reader error 映射 500。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:268-275` |
...
它是给 SimpleTrack SaaS 服务端页面读回数据的内部接口。`/collect` 负责收事件，`/v1/realtime` 和 `/v1/events` 负责把已入库的事件读出来给产品页面展示。因为它能读用户分析数据，所以必须放在 internal query token 后面。
...
读接口需要两个值：query token 证明“你有资格读”，write key 决定“你要读哪个 source”。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:194-233`

### Realtime 和 Events 查询的区别

Realtime 是最近窗口，默认最近 30 分钟，默认 limit 50，适合页面上“刚刚有没有收到事件”。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:17-22`、`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:90-119`

Events 是明确时间范围内的分页列表，必须传 `from` 和 `to`，支持 `event_name`、`distinct_id`、排序、分页和 property filter。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:140-191`

### SourceConfig 为什么仍然要参与读接口

因为读接口不能相信客户端传 tenant/project/source。它只接受 write key，然后用 resolver 查出 SourceConfig，再把 SourceConfig 的 tenant/project/source 放进 query object。这样读写两条链路都使用同一套 source 边界。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:212-233`、`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:101-109`、`仓库: analytics-service, commit: 825e366c014b, file: internal/collectapi/query.go:163-179`

### property filter 为什么要白名单

property filter 最终会影响 ClickHouse 查询。如果任意 property name 都能查，既可能造成高成本扫描，也可能让产品界面暴露尚未设计好的属性面。当前实现要求 source config 明确允许某个 `scope + name + value type`，并且 builder 再检查 selector。证据：`仓库: analytics-service, commit: 825e366c014b, file: internal/controlplane/resolver.go:80-108`、`仓库: analytics-core, commit: 58668c9c17ea, file: storage/clickhouse/query_builder.go:257-332`
