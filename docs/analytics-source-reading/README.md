...
| 子仓 | commit id | 说明 |
| --- | --- | --- |
| `src/analytics-core` | `770610c6079f8bfc1eb30f8d747dd466874955ab` | P1 数据管道、`visit_id` 持久化、Events/Realtime query builder、P1.5 query evidence、property catalog 基础契约、cataloging writer、builder-only benchmark、真实 ClickHouse EventReader / BatchWriter benchmark、Redis Stream publish / subscribe+ack benchmark、`collect.Handler` 热路径 benchmark、GORM `CreateInBatches` 对照 benchmark，以及读侧优化引入门槛策略均已收口；历史章节内保留早期 commit 引用作为当时源码证据 |
...
- `analytics-service` 的 readback 响应现在会透出 `query_evidence`，帮助判断 query family、read path、optimization、过滤数量、属性表参与情况和初始 pressure 分档；其中 `pressure` 只是 triage 桶，不是 SLA，也不是自动扩缩容信号。它们来自 `analytics-core` 的 `EventQueryPlan.QueryEvidence()`，不是 SQL 文本。
