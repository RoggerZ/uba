...
| `src/simpletrack-saas` | `cb643e3db7a7bec0bec7f861261143c842b942f6` | runtime-source API、Websites 控制面、Realtime/Events/Goals server-side readback helper、Events 属性目录读回、filter builder 接入、Goal 数据模型、并发唯一约束回归、`duplicate_event` 冲突映射、Goals readback UI 的 true no-data / service failure 状态拆分、Goal event-name contract samples、Goal readback fan-out 的 25 条上限、runtime-source `readback_policy` 显式契约，以及 authenticated readback 页面 selector / 空态 / 错误态收口均已完成；`9ebe9f4` 先把 Next dev `allowedDevOrigins` 与 Better Auth loopback trusted origins 收口到共享 helper，`ffdb254` 再把 Websites 页面 UI 收口为紧凑来源行 + write key 面板 + accordion 设置区，`b1db9d5` 又让 server-side readback helper 按 `realtime / events / properties / goals` 优先选择 route-scoped query token，`08b9acc` 进一步把本地 auth/mail runtime 收口为 development 默认不强制 email verification、无 mail provider 时仅在 development 走 console fallback，`2a33f00` 又把 authenticated Realtime / Events / Goals 的 no-source / disabled-source 产品态收口为共享 setup empty state、shared enabled-source resolver 和显式 fallback notice，`075828d` 再把 empty state 收口为 `Websites + Quickstart` 双动作并把 disabled-source fallback 深链统一收口到 `buildWebsiteSettingsHref()`，`10697eb` 补上真实 Playwright e2e，覆盖 sign-up、sign-in、onboarding、organization create、Websites create、Realtime / Events / Goals setup empty state、disabled-source fallback 与 `edit_target` 展开，`d48660e` 把 Quickstart CTA 变成真实点击验证，`b3d2904` 又给 docs quickstart 可见内容补上稳定 test id，`aff924e` 再把这条真实点击验证从 Realtime 扩展到 Events 和 Goals，而最新 `cb643e3` 又把 disabled-source fallback 的浏览器验证从 Realtime 扩展到 Events 和 Goals，让三个分析入口页的共享 fallback contract 也由浏览器回归锁住；目标 `oxfmt --check`、`saas` type-check 和目标 Playwright 回归均已通过 |
...
- `simpletrack-saas` 现在还有真实 authenticated Playwright 回归兜底：signup -> sign-in -> onboarding -> organization create -> Websites / Realtime / Events / Goals setup empty state -> Quickstart docs cross-origin navigation -> disabled-source fallback -> `edit_target` deep link expansion 已由浏览器级测试锁住，且测试不再依赖直接修改 Postgres 用户状态。
