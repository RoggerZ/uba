...
- `simpletrack-saas` 现在还有真实 authenticated Playwright 回归兜底：signup -> sign-in -> onboarding -> organization create -> Websites / Realtime / Events / Goals setup empty state -> disabled-source fallback -> `edit_target` deep link expansion 已由浏览器级测试锁住，且测试不再依赖直接修改 Postgres 用户状态。
