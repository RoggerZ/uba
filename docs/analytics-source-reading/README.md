...
| 子仓 | commit id | 说明 |
| --- | --- | --- |
| `src/analytics-core` | `979a29fcfd6a09ee4433a5e8f42d97ddb247dbc1` | P1 数据管道、`visit_id` 持久化、Events/Realtime query builder、P1.5 query evidence、property catalog 基础契约、cataloging writer、builder-only benchmark、真实 ClickHouse EventReader / BatchWriter benchmark、Redis Stream publish / subscribe+ack benchmark、`collect.Handler` 热路径 benchmark、GORM `CreateInBatches` 对照 benchmark、读侧优化引入门槛策略，以及 limit / offset / time window 查询形状证据均已收口；历史章节内保留早期 commit 引用作为当时源码证据 |
...
- `analytics-service` 的 readback 响应现在会透出 `query_evidence`，帮助判断 query family、read path、optimization、effective limit、offset、time lower/upper bound、time window、过滤数量、属性表参与情况和初始 pressure 分档；其中 `pressure` 只是 triage 桶，不是 SLA，也不是自动扩缩容信号。它们来自 `analytics-core` 的 `EventQueryPlan.QueryEvidence()`，不是 SQL 文本。
...
- 当前第二步实现是 `EventQueryEvidence`：`storage.EventQueryPlan.QueryEvidence()` 会暴露 query family、read path、optimization、effective limit、offset、time lower/upper bound、time window、filter count、property table usage 和 sort evidence，用来支持后续读侧取舍；`pressure` 只是这组证据的派生分档，当前分为 low / medium / high 三档。
...
- 代码证据：`EventQueryEvidence` / `QueryEvidence()` 位于 `仓库: analytics-core, commit: ee455ac, file: storage/event_query.go:132-172`；ClickHouse evidence 生成位于 `仓库: analytics-core, commit: ee455ac, file: storage/clickhouse/query_builder.go:388-411`；read-side shape benchmark 位于 `仓库: analytics-core, commit: 15f7ea9, file: storage/clickhouse/query_builder_benchmark_test.go:1-181`；真实 ClickHouse EventReader benchmark 位于 `仓库: analytics-core, commit: eeb63c6, file: internal/e2e/clickhouse_reader_benchmark_test.go:1-544`；真实 ClickHouse BatchWriter benchmark 位于 `仓库: analytics-core, commit: 26762c6, file: internal/e2e/clickhouse_writer_benchmark_test.go:1-149`；GORM / native 写入对照 benchmark 位于 `仓库: analytics-core, commit: c868b5d, file: internal/e2e/clickhouse_writer_benchmark_test.go:102-492`；Redis Stream benchmark 位于 `仓库: analytics-core, commit: 0bbfd08, file: eventbus/redisstream/redisstream_benchmark_test.go:1-235`；collect handler benchmark 位于 `仓库: analytics-core, commit: d1f0516, file: collect/handler_benchmark_test.go:16-224`；pressure triage 常量、OpenAPI 描述与测试位于 `仓库: analytics-service, commit: 5b4cd9a, file: internal/collectapi/query.go:79-82`、`api/openapi.yaml:419-422` 和 `internal/collectapi/handler_test.go:553-595`。
- 读侧优化策略证据：`仓库: analytics-core, commit: 770610c, file: docs/read-side-optimization-policy.md:12-30` 定义必须提交的证据材料；`docs/read-side-optimization-policy.md:32-58` 定义 direct fact table、projection、materialized view、小时聚合表的适用边界和初始门槛；`docs/read-side-optimization-policy.md:72-98` 记录标准测试、读侧 benchmark 和写入对照 benchmark 命令。
- 查询形状证据：`仓库: analytics-core, commit: 979a29f, file: storage/event_query.go:133-145` 定义 evidence 字段；`storage/clickhouse/query_builder.go:390-417` 从归一化 query plan 填充 effective limit、offset 和时间窗；`storage/clickhouse/query_builder_test.go:79-89`、`139-152`、`403-413` 覆盖 Events、Realtime 和复杂属性查询；`storage/clickhouse/query_builder_benchmark_test.go:90-193` 把 benchmark 场景绑定到新的 evidence 形状。`仓库: analytics-service, commit: 3d858bf, file: internal/collectapi/query.go:67-83` 透出 readback 响应字段；`api/openapi.yaml:386-443` 同步 OpenAPI schema；`internal/collectapi/handler_test.go:404-414` 和 `561-571` 验证序列化后的 Realtime / Events evidence。
