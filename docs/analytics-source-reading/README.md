...
| `src/simpletrack-saas` | `2a33f0074eab8e67c6082bdbb84a488d79034191` | runtime-source API、Websites 控制面、Realtime/Events/Goals server-side readback helper、Events 属性目录读回、filter builder 接入、Goal 数据模型、并发唯一约束回归、`duplicate_event` 冲突映射、Goals readback UI 的 true no-data / service failure 状态拆分、Goal event-name contract samples、Goal readback fan-out 的 25 条上限、runtime-source `readback_policy` 显式契约，以及 authenticated readback 页面 selector / 空态 / 错误态收口均已完成；`9ebe9f4` 先把 Next dev `allowedDevOrigins` 与 Better Auth loopback trusted origins 收口到共享 helper，`ffdb254` 再把 Websites 页面 UI 收口为紧凑来源行 + write key 面板 + accordion 设置区，`b1db9d5` 又让 server-side readback helper 按 `realtime / events / properties / goals` 优先选择 route-scoped query token，`08b9acc` 进一步把本地 auth/mail runtime 收口为 development 默认不强制 email verification、无 mail provider 时仅在 development 走 console fallback，而最新 `2a33f00` 又把 authenticated Realtime / Events / Goals 的 no-source / disabled-source 产品态收口为共享 setup empty state、shared enabled-source resolver 和显式 fallback notice；目标 Vitest（140 tests）、`saas` type-check、oxfmt、Codex native reviewer `APPROVE` 与 architect `CLEAR` 均已完成 |
...
- authenticated Realtime / Events / Goals 的页面壳现在也更接近真实 P1：当组织没有启用 Website source 时，页面保留 selector 与上下文说明，但主内容区统一切换到 setup empty state；当深链请求命中 disabled source 且组织里还有其他 enabled source 时，页面会显式展示 fallback notice，而不是静默切到另一套数据。
