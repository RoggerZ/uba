...
- `simpletrack-saas` 的 Websites onboarding 现在更直接：当 source 创建成功后，页面顶部 alert 会给出 `Open Quickstart`，source row 的 write-key card 也会给出持久化 `Quickstart`；两者都要求新标签页外链语义，且 docs URL 缺失时不渲染 CTA，避免把用户带去错误地址。这个契约已经由 `websites-page.test.tsx` 和 `simpletrack-analytics-empty-state.spec.ts` 分别从 SSR 与浏览器级锁住。
