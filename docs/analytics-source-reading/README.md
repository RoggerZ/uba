...
| 子仓 | commit id | 说明 |
| --- | --- | --- |
| `src/analytics-core` | `c868b5d59e534a95cfa5f7b5ee0c72f999ae6160` | P1 数据管道、`visit_id` 持久化、Events/Realtime query builder、P1.5 query evidence、property catalog 基础契约、cataloging writer、builder-only benchmark、真实 ClickHouse EventReader / BatchWriter benchmark、Redis Stream publish / subscribe+ack benchmark、`collect.Handler` 热路径 benchmark，以及 GORM `CreateInBatches` 对照 benchmark 均已收口；历史章节内保留早期 commit 引用作为当时源码证据 |
