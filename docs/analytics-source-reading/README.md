...
| 子仓 | commit id | 说明 |
| --- | --- | --- |
| `src/analytics-core` | `d1f05167878373d9a64631ac924d3b47847145d9` | P1 数据管道、`visit_id` 持久化、Events/Realtime query builder、P1.5 query evidence、property catalog 基础契约、cataloging writer、builder-only benchmark、真实 ClickHouse EventReader / BatchWriter benchmark、Redis Stream publish / subscribe+ack benchmark 和 `collect.Handler` 热路径 benchmark 均已收口；历史章节内保留早期 commit 引用作为当时源码证据 |
