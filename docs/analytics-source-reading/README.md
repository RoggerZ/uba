...
本目录已同步到 2026-05-11 的 P1 收口状态。源码引用基线：

| 子仓 | commit id | 说明 |
| --- | --- | --- |
| `src/analytics-core` | `a99147f4da07ccfd6643722a891cad65c1270b3e` | P1 数据管道、`visit_id` 持久化、Events/Realtime query builder、P1.5 query evidence、property catalog 基础契约、cataloging writer、source-scoped catalog reader、Goal count read-side 契约、builder-only benchmark、真实 ClickHouse EventReader / BatchWriter benchmark、Redis Stream publish / subscribe+ack benchmark、`collect.Handler` 热路径 benchmark、GORM `CreateInBatches` 对照 benchmark、读侧优化引入门槛策略、ClickHouse explain 证据、limit / offset / time window 查询形状证据，以及 value-free property filter shape evidence 均已收口；`QueryEvidence()` 已补快照语义；`collect.ValidateEventName` 现在有 accepted/rejected contract samples，和 SaaS Goal 校验样例一一对齐；typed property filters 已收口到 query-builder guardrail：必须显式带 `from/to`，且 direct fact-table 查询窗口默认不超过 7 天；最新又把 bounded scalar Events benchmark / explain 从单一 24h 扩展为 24h / 72h / 7d 多窗口证据梯度，避免后续只用一个时间窗样本讨论 bounded scalar pressure |
| `src/analytics-service` | `580c547a29546ff9f0dd7314e075a7deff3d8412` | Fiber runtime、`/collect`、`/tracker.js`、内部 `/v1/realtime` / `/v1/events` / `/v1/properties` / `/v1/goals`、HTTP resolver、readback API、ingestion 属性目录运行时装配、property metadata readback、Goal count readback、query shape evidence 透出、Goal 服务层回归，以及 source-scoped `readback_policy` fail-closed 路由控制均已收口；`property_filters` 只透出 scope/name/value_type/operator，不返回属性值；pressure 仅作为 read-side triage 桶公开；`6f6f742` 先用真实 `analytics-core` query builder 把 typed property filter 的 7 天窗口护栏锁到 `/v1/events` handler 回归，`c08e1da` 又根据 100k / 500k bounded 24h scalar Events 证据撤回了 `24h => high` 的 bounded scalar heuristic，让 pressure bucket 回到 filter-count 主导；最新 `580c547` 又给内部 query token 增加 readback route scope 第二道权限层，在 source 解析前先按 `realtime / events / properties / goals` 判定 token 是否有资格访问对应读回放族 |
| `src/simpletrack-saas` | `9d3bbc8b61c8d703ab6f956a8543c425af03175a` | runtime-source API、Websites 控制面、Realtime/Events/Goals server-side readback helper、Events 属性目录读回、filter builder 接入、Goal 数据模型、并发唯一约束回归、`duplicate_event` 冲突映射、Goals readback UI 的 true no-data / service failure 状态拆分、Goal event-name contract samples、Goal readback fan-out 的 25 条上限、runtime-source `readback_policy` 显式契约，以及 authenticated readback 页面 selector / 空态 / 错误态收口均已完成；`9ebe9f4` 先把 Next dev loopback trusted origins 收口到共享 helper，`ffdb254` 再把 Websites 页面 UI 收口为紧凑来源行 + write key 面板 + accordion 设置区，`b1db9d5` 让 server-side readback helper 优先选择 route-scoped query token，`08b9acc` 把本地 auth/mail runtime 收口为 development 默认不强制 email verification，`2a33f00` 把 authenticated Realtime / Events / Goals 的 no-source / disabled-source 产品态收口为共享 setup empty state、shared enabled-source resolver 和显式 fallback notice，`d48660e`、`aff924e`、`cb643e3` 与 `6a14cd9` 继续把 Quickstart、disabled-source fallback 和 analytics-service outage 纳入真实浏览器回归，`d538e4a` 又把 `apps/saas` / `apps/docs` 的 Next.js workspace root 显式锁到 `simpletrack-saas` 工作区，并让 docs `metadataBase` 只在可信运行时 origin 下生效，避免生产元数据回退到 localhost；`edc7cbd` 继续把 Events 页在 analytics-service outage 下仍可操作，以及 Websites disabled source 的 source-scoped 直达链接撤销契约锁进同一条 authenticated Playwright 回归；`246954d` 又把 property catalog 失败时的半失败态提示补到 Events 页，确保 raw events 仍可读时不会把页面打回 setup 态；`b9a2d72` 再把同一半失败态下 Raw events 数据表与事件行继续可见的契约锁进 SSR 回归；`6a3d677` 把 Realtime non-ready false-zero 风险和 Events 全 outage duplicate backend error copy 收口到 SSR + Playwright 回归；`8eab468` 又把 Goals 全 outage duplicate backend error copy 收口为共享 page-level warning + row-level placeholder；`b8a8e01` 已把 Website source 创建成功后的 `Open Quickstart` alert CTA 与 source row `Quickstart` CTA 同时锁进 SSR + Playwright 回归；`0760713` 再把共享 setup state 与 Websites 创建区内联 Quickstart helper link 的新标签页语义补齐到同一标准，并把这条 helper-link 覆盖直接补进真实浏览器回归；`829592b` 又把 helper CTA / alert CTA accessible name 区分、`quickstart-docs-page` 脆弱可见性检查替换为 URL + visible step marker 验证，并把 Quickstart popup contract 锁得更稳；最新 `9d3bbc8` 再把 analytics readback helper 的 property catalog mock 收口为 route-scoped service URL、为 Playwright mock 补 `/healthz` readiness，并把 Events `Property suggestions unavailable` 与筛选控件可见性的浏览器断言补进同一闭环；目标 `corepack pnpm --filter saas test -- analytics-readback-core.test.ts`、`corepack pnpm --filter saas exec tsc --noEmit`、`corepack pnpm --filter saas exec playwright test tests/simpletrack-analytics-empty-state.spec.ts --project chromium --reporter=dot` 均已通过 |
...
- `simpletrack-saas` 的本地 authenticated 开发闭环现在不再依赖 live email：development 默认不强制 email verification，且 console mail fallback 只在 development 生效；这条规则用于本机 Docker Postgres + 本地浏览器 / curl 评审，不改变 production 默认策略。
- authenticated Realtime / Events / Goals 的页面壳现在也更接近真实 P1：当组织没有启用 Website source 时，页面保留 selector 与上下文说明，但主内容区统一切换到 setup empty state，并提供 `Websites + Quickstart` 双动作；当深链请求命中 disabled source 且组织里还有其他 enabled source 时，页面会显式展示 fallback notice，并深链到对应 source 的 Websites 设置折叠区，而不是静默切到另一套数据。
- `simpletrack-saas` 现在还有真实 authenticated 回归兜底：signup -> sign-in -> onboarding -> organization create -> Websites / Realtime / Events / Goals setup empty state -> Quickstart docs cross-origin navigation -> disabled-source fallback -> `edit_target` deep link expansion -> analytics-service outage 下 Realtime 不再伪装 `0/0` 指标、Events 页仍保留 `Raw events` 与筛选控件且全 outage 只显示一条 warning -> disabled source 撤销 source-scoped 直达读回放链接，已由浏览器级测试锁住；与此同时，property catalog service_error 也已经被服务端渲染回归锁住，保证属性建议失败时仍保留可操作的 Events 筛选 UI，且测试不再依赖直接修改 Postgres 用户状态。
- `simpletrack-saas` 的 Websites onboarding 现在更直接：当 source 创建成功后，页面顶部 alert 会给出 `Open Quickstart`，source row 的 write-key card 也会给出持久化 `Quickstart`；共享 setup state 和 Websites 创建区内联 helper copy 里的 Quickstart 也已经统一到同一套新标签页外链语义，且 docs URL 缺失时不渲染 CTA，避免把用户带去错误地址。`829592b` 又把两个 Quickstart 外链的 accessible name 分离，并把 docs popup 的验收从脆弱空 wrapper 改成 URL + 可见 step marker；最新 `9d3bbc8` 则把 property catalog fail-soft 的 route-scoped mock、`/healthz` readiness 和 Events `Property suggestions unavailable` 浏览器断言补进同一条 authenticated 回归。这个契约已经由 `websites-page.test.tsx` 和 `simpletrack-analytics-empty-state.spec.ts` 分别从 SSR 与浏览器级锁住。
...
- `query token` 现在不再只是“Bearer token + 生效/过期时间”这一层。`analytics-service` `580c547` 给 token 增加了可选 route scopes：空 scope 仍表示兼容旧行为，可访问所有内部读回放路由；如果配置了 `realtime / events / properties / goals` 子集，服务会在 source resolution 之前先判定 token 是否允许访问该 route family，不允许时直接返回 `403 forbidden`。
...
- `analytics-service` 不再把“仅仅 bounded 24h 的 scalar Events”单独抬到 `pressure=high`。当前的 bucket 再次只反映过滤复杂度：无 property filter 且标量条件很少是 `low`，常规 scalar/property 组合仍按 `medium/high` 走。
- `analytics-core` 现在也把 benchmark hygiene 钉到了代码里：bounded 24h scalar Events 只在 fixture 真大于一天时进入 benchmark / explain 套件，默认 10k 本地运行不会再生成一个与 wide-window 等价的伪 24h 分支。
- 已补 100k 行 bounded 24h scalar Events 的真实 ClickHouse explain / benchmark：当前 `eligible_rows=86400`、`time_window_seconds=86400`、`Granules: 12/13`，三次 benchmark 约 `10.8ms/op`、`12.6ms/op`、`15.5ms/op`。它已经是独立读形状，但仍未单独证明必须新增 projection / materialized view / 小时聚合表。
- `analytics-core` `a99147f` 又把 bounded scalar 证据扩成多窗口套件：`72h` 在 500k 行夹具下是 `eligible_rows=259200`、`Granules: 33/62`、约 `10.3-11.5ms/op`；`7d` 在 1,000,000 行夹具下是 `eligible_rows=604800`、`Granules: 75/123`、约 `46.1-52.1ms/op`。这说明 bounded scalar 已经有了从 24h 到 7d 的更完整压力梯度，但当前仍不自动触发 service heuristic 或新的 ClickHouse 物理结构。
...
- 当前第六步记录真实 ClickHouse reader benchmark 基线：2026-05-08 到 2026-05-10 在本机 `clickhouse/clickhouse-server:25.3` 上执行 `BenchmarkEventReaderClickHouseExecution`，10k 行下 low realtime 约 8.6-10.4ms/op，medium scalar events 约 8.6-9.7ms/op，high property events 约 15.2-16.3ms/op；100k 行 pressure run 两轮下 high property 稳定在约 29.8-34.2ms/op；100k 行 bounded 24h scalar Events 三次约 `10.8-15.5ms/op`；500k 行 bounded 72h scalar Events 三次约 `10.3-11.5ms/op`；500k 行复测后，短窗口 `low_realtime_recent_window` 稳定在约 7.9-9.0ms/op，宽时间窗 `low_realtime_wide_since` 约 37.4-54.7ms/op，recent-window scalar Events 约 8.2-9.0ms/op，wide-window scalar Events 约 40.3-42.1ms/op，recent-window typed property Events 约 21.4-23.4ms/op，wide-window typed property Events 约 43.0-44.1ms/op；1,000,000 行 bounded 7d scalar Events 三次约 `46.1-52.1ms/op`；详见 [read-side-benchmark-baseline.md](read-side-benchmark-baseline.md)。
...
- 代码证据：`EventQueryEvidence` / `EventPropertyFilterEvidence` 位于 `仓库: analytics-core, commit: b693d62, file: storage/event_query.go:84-94` 和 `storage/event_query.go:149-163`；`NewEventQueryPlan` / `QueryEvidence()` 的 snapshot copy 位于 `storage/event_query.go:181-203`，不可变性回归和 value-free example 位于 `storage/event_query_test.go:5-39`、`storage/event_query_example_test.go:9-33`；ClickHouse evidence 生成位于 `仓库: analytics-core, commit: b693d62, file: storage/clickhouse/query_builder.go:390-451`；read-side shape benchmark 位于 `仓库: analytics-core, commit: b693d62, file: storage/clickhouse/query_builder_benchmark_test.go:1-222`；真实 ClickHouse EventReader benchmark 的多窗口 bounded scalar suite、Realtime / Events shape 和精确排他上界位于 `仓库: analytics-core, commit: a99147f, file: internal/e2e/clickhouse_reader_benchmark_test.go:22-264`，真实 explain 套件对齐位于 `internal/e2e/clickhouse_reader_benchmark_test.go:267-404`，bounded scalar suite helpers 位于 `internal/e2e/clickhouse_reader_benchmark_test.go:443-531`；真实 ClickHouse BatchWriter benchmark 位于 `仓库: analytics-core, commit: 26762c6, file: internal/e2e/clickhouse_writer_benchmark_test.go:1-149`；GORM / native 写入对照 benchmark 位于 `仓库: analytics-core, commit: c868b5d, file: internal/e2e/clickhouse_writer_benchmark_test.go:102-492`；Redis Stream benchmark 位于 `仓库: analytics-core, commit: 0bbfd08, file: eventbus/redisstream/redisstream_benchmark_test.go:1-235`；collect handler benchmark 位于 `仓库: analytics-core, commit: d1f0516, file: collect/handler_benchmark_test.go:16-224`；pressure triage、`property_filters` 响应转换、OpenAPI 描述与测试位于 `仓库: analytics-service, commit: 64b0bda, file: internal/collectapi/query.go:83-106`、`internal/collectapi/query.go:671-710`、`api/openapi.yaml:466-538` 和 `internal/collectapi/handler_test.go:573-668`。
- service 边界护栏证据：`仓库: analytics-service, commit: c08e1da, file: internal/collectapi/handler_test.go:1001-1152` 覆盖 `/v1/events` 的 7 天 typed property filter success / 8 天 rejection；`internal/collectapi/handler_test.go:1968-2011` 定义 `corePlanningQueryReader`，把 handler 映射后的 `EventListQuery` 送进真实 `analytics-core` ClickHouse query builder，只做 planning 不做本地 ClickHouse 执行，从而验证 service boundary 没有绕过 core guardrail。
- bounded scalar pressure 调整证据：`仓库: analytics-service, commit: c08e1da, file: internal/collectapi/query.go:819-833` 已移除 `24h => high` 的 bounded scalar 特判；`internal/collectapi/handler_test.go:761-804` 把裸 `EventQueryEvidence` bucket 回归改成 filter-count 主导；`internal/collectapi/handler_test.go:1038-1098` 再通过真实 `analytics-core` planner 回归验证 wide scalar 保持 `medium`、`from/to`-only exact 24h bounded query 回到 `low`、recent scalar 维持 `medium`。
...
- explain 证据入口：`仓库: analytics-core, commit: a99147f, file: internal/e2e/clickhouse_reader_benchmark_test.go:267-404` 定义 `TestEventReaderClickHouseExplain`，按 24h / 72h / 7d 多窗口规则加入 bounded scalar 场景，并记录 Realtime / Events eligible row evidence；`internal/e2e/clickhouse_reader_benchmark_test.go:773-795` 说明 explain 直接复用 sealed query plan SQL 和 bound args，不额外拼装第二套 SQL。
